...
int main(int argc, char **argv) {
	...
	if(mode == CREATE_HTML && display_header == TRUE) {
		...
		if(display_type != DISPLAY_NO_TRENDS && input_type == GET_INPUT_NONE) {
			...
			if(display_type == DISPLAY_HOST_TRENDS) {
				...
				printf("<a href='%s?host=%s'>%s</a><BR>\n", NOTIFICATIONS_CGI, url_encode(host_name),_("View Alert History For This Host"));
				}
			else {
				printf("<a href='%s?host=%s&t1=%lu&t2=%lu&includesoftstates=%s&assumestateretention=%s&assumeinitialstates=%s&assumestatesduringnotrunning=%s&initialassumedservicestate=%d&backtrack=%d'>%s</a><BR>\n", TRENDS_CGI, url_encode(host_name), t1, t2, (include_soft_states == TRUE) ? "yes" : "no", (assume_state_retention == TRUE) ? "yes" : "no", (assume_initial_states == TRUE) ? "yes" : "no", (assume_states_during_notrunning == TRUE) ? "yes" : "no", initial_assumed_service_state, backtrack_archives,_("View Trends For This Host"));
				printf("<a href='%s?host=%s", AVAIL_CGI, url_encode(host_name));
				printf("&service=%s&t1=%lu&t2=%lu&assumestateretention=%s&includesoftstates=%s&assumeinitialstates=%s&assumestatesduringnotrunning=%s&initialassumedservicestate=%d&backtrack=%d&show_log_entries'>%s</a><BR>\n", url_encode(svc_description), t1, t2, (include_soft_states == TRUE) ? "yes" : "no", (assume_state_retention == TRUE) ? "yes" : "no", (assume_initial_states == TRUE) ? "yes" : "no", (assume_states_during_notrunning == TRUE) ? "yes" : "no", initial_assumed_service_state, backtrack_archives,_("View Availability Report For This Service"));
		...
		if(display_type != DISPLAY_NO_TRENDS && input_type == GET_INPUT_NONE) {
			...
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>%s:</td><td CLASS='optBoxItem' valign=top align=left>%s:</td></tr>\n",_("Report period"),_("Zoom factor"));
			printf("<tr><td CLASS='optBoxItem' valign=top align=left>\n");
			printf("<select name='timeperiod'>\n");
			printf("<option>[ %s ]\n",_("Current time range"));
			printf("<option value=today %s>%s\n", (timeperiod_type == TIMEPERIOD_TODAY) ? "SELECTED" : "",_("Today"));
			printf("<option value=last24hours %s>%s\n", (timeperiod_type == TIMEPERIOD_LAST24HOURS) ? "SELECTED" : "",_("Last 24 Hours"));
			printf("<option value=yesterday %s>%s\n", (timeperiod_type == TIMEPERIOD_YESTERDAY) ? "SELECTED" : "",_("Yesterday"));
			printf("<option value=thisweek %s>%s\n", (timeperiod_type == TIMEPERIOD_THISWEEK) ? "SELECTED" : "",_("This Week"));
			printf("<option value=last7days %s>%s\n", (timeperiod_type == TIMEPERIOD_LAST7DAYS) ? "SELECTED" : "",_("Last 7 Days"));
			printf("<option value=lastweek %s>%s\n", (timeperiod_type == TIMEPERIOD_LASTWEEK) ? "SELECTED" : "",_("Last Week"));
			printf("<option value=thismonth %s>%s\n", (timeperiod_type == TIMEPERIOD_THISMONTH) ? "SELECTED" : "",_("This Month"));
			printf("<option value=last31days %s>%s\n", (timeperiod_type == TIMEPERIOD_LAST31DAYS) ? "SELECTED" : "",_("Last 31 Days"));
			printf("<option value=lastmonth %s>%s\n", (timeperiod_type == TIMEPERIOD_LASTMONTH) ? "SELECTED" : "",_("Last Month"));
			printf("<option value=thisyear %s>%s\n", (timeperiod_type == TIMEPERIOD_THISYEAR) ? "SELECTED" : "",_("This Year"));
			printf("<option value=lastyear %s>%s\n", (timeperiod_type == TIMEPERIOD_LASTYEAR) ? "SELECTED" : "",_("Last Year"));
			if(display_type == DISPLAY_HOST_TRENDS)
	...
	if(is_authorized == FALSE) {
		...
		if(mode == CREATE_HTML)
			printf("<P><DIV ALIGN=CENTER CLASS='errorMessage'>%s %s...</DIV></P>\n", _("It appears as though you are not authorized to view information for the specified"),(display_type == DISPLAY_HOST_TRENDS) ? "host" : "service");
	...
	if(timeperiod_type == TIMEPERIOD_NEXTPROBLEM && problem_found == FALSE) {
		printf("<P><DIV ALIGN=CENTER CLASS='errorMessage'>%s</DIV></P>\n",_("No problem found between end of display and end of recording"));
...
#ifdef DEBUG
				printf("%s\n",_("Error: Could not allocate memory for image"));
#endif
				return ERROR;
				}
			}
	...
	if(display_type != DISPLAY_NO_TRENDS && input_type == GET_INPUT_NONE) {
		...
		if(mode == CREATE_IMAGE) {
			...
			if(image_file == NULL)
				printf(_("Could not open trends.png for writing!\n"));
			else {
				gdImagePng(trends_image, image_file);
				fclose(image_file);
				}
	...
	if(display_type == DISPLAY_NO_TRENDS || input_type != GET_INPUT_NONE) {
		...
		if(input_type == GET_INPUT_HOST_TARGET) {
			...
			printf("<DIV CLASS='reportSelectTitle'>%s</DIV>\n",_("Step 2: Select Host"));
			printf("</DIV></P>\n");
			...
			printf("<tr><td class='reportSelectSubTitle' valign=center>%s:</td>\n",_("Host"));
		...
		/* ask the user for what service they want a report for */
			...
			printf("<tr><td class='reportSelectSubTitle'>%s:</td>\n",_("Service"));
		...
		/* ask the user for report range and options */
			...
			printf("<tr><td class='reportSelectSubTitle' align=right>%s:</td><td class='reportSelectItem'><input type='checkbox' name='nomap'></td></tr>",_("Suppress image map"));
			printf("<tr><td class='reportSelectSubTitle' align=right>%s:</td><td class='reportSelectItem'><input type='checkbox' name='nopopups'></td></tr>\n",_("Suppress popups"));

			printf("<tr><td></td><td class='reportSelectItem'><input type='submit' value='%s'></td></tr>\n",_("Create Report"));
			...
			printf(_("Note: Choosing the 'suppress image map' option will make the report run approximately twice as fast as it would otherwise, but it will prevent you from being able to zoom in on specific time periods.\n"));
			printf("</DIV></P>\n");
			*/
			}
		...
		/* as the user whether they want a graph for a host or service */
			...
			printf("<option value=gethost>%s\n",_("Host"));
			printf("<option value=getservice>%s\n",_("Service"));
			printf("</select>\n");
			printf("</td></tr>\n");

			printf("<tr><td></td><td class='reportSelectItem'>\n");
			printf("<input type='submit' value='%s'>\n",_("Continue to Step 2"));
			printf("</td></tr>\n");
...
#ifdef DEBUG
			printf("%s=%d<br>\n",_("SETTING LAST KNOWN STATE"), last_known_state);
#endif
			}
...
#ifdef DEBUG
		printf(_("First state=program start!\n"));
#endif
		if(assume_initial_states == TRUE) {
#ifdef DEBUG
			printf(_("\tWe are assuming initial states...\n"));
#endif
			if(assume_state_retention == TRUE) {
				start_state = last_known_state;
#ifdef DEBUG
				printf("\t%s (%d)...\n",_("We are assuming state retention"), start_state);
#endif
				}
			else {
#ifdef DEBUG
				printf("\t%s...\n",_("We are NOT assuming state retention"));
#endif
				if(display_type == DISPLAY_HOST_TRENDS)
...
#ifdef DEBUG
			printf(_("We ARE NOT assuming initial states!\n"));
#endif
			return;
			}
		}
...
#ifdef DEBUG
	printf("%s %d\n",_("Graphing state") ,start_state);
	printf("\t%s %s",_("from"), ctime(&start_time));
	printf("\t%s %s", _("to"),ctime(&end_time));
#endif
...
#ifdef DEBUG
	printf("%s %d @ %s", _("Added state"),state_type, ctime(&time_stamp));
#endif
...
#ifdef DEBUG
	printf(_("Determining archives to use...\n"));
#endif
...
#ifdef DEBUG
	printf("%s: %d\n", _("Oldest archive"),oldest_archive);
	printf("%s: %d\n", _("Newest archive"), newest_archive);
#endif
...
#ifdef DEBUG
		printf("\t%s: %d (%s)\n", _("Current archive"),current_archive, filename);
#endif
