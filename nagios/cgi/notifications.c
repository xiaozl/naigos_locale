	...
	if(display_header == TRUE) {
		...
		if(query_type == FIND_HOST || query_type == FIND_SERVICE) {
			...
			if(query_type == FIND_HOST) {
				printf("<A HREF='%s?host=%s'>%s %s</A><BR>\n", STATUS_CGI, (find_all == TRUE) ? "all" : url_encode(query_host_name), _("View Status Detail For"),(find_all == TRUE) ? "All Hosts" : "This Host");
				printf("<A HREF='%s?host=%s'>%s %s</A><BR>\n", HISTORY_CGI, (find_all == TRUE) ? "all" : url_encode(query_host_name), _("View History For"),(find_all == TRUE) ? "All Hosts" : "This Host");
#ifdef USE_TRENDS
				if(find_all == FALSE)
					printf("<A HREF='%s?host=%s'>%s</A><BR>\n", TRENDS_CGI, url_encode(query_host_name),_("View Trends For This Host"));
#endif
				}
			else if(query_type == FIND_SERVICE) {
				printf("<A HREF='%s?host=%s&", HISTORY_CGI, (find_all == TRUE) ? "all" : url_encode(query_host_name));
				printf("service=%s'>%s</A><BR>\n", url_encode(query_svc_description),_("View History For This Service"));
#ifdef USE_TRENDS
				printf("<A HREF='%s?host=%s&", TRENDS_CGI, (find_all == TRUE) ? "all" : url_encode(query_host_name));
				printf("service=%s'>%s</A><BR>\n", url_encode(query_svc_description),_("View Trends For This Service"));
#endif
				}
		...
		if(query_type == FIND_SERVICE)
			...
			if(find_all == TRUE)
				printf("%s",_("All Hosts and Services"));
			else
				printf("%s '%s'",_("Host"), query_host_name);
			}
		else {
			if(find_all == TRUE)
				printf(_("All Contacts"));
			else
				printf("%s '%s'",_("Contact"), query_contact_name);
			}
		printf("</DIV>\n");
		printf("<BR>\n");
		...
		printf("<tr>\n");
		if(query_type == FIND_SERVICE)
			printf("<td align=left colspan=2 CLASS='optBoxItem'>%s:</td>",_("Notification detail level for this service"));
		else
			printf("<td align=left colspan=2 CLASS='optBoxItem'>%s %s %s%s:</td>", _("Notification detail level for"),(find_all == TRUE) ? "all" : "this", (query_type == FIND_HOST) ? "host" : "contact", (find_all == TRUE) ? "s" : "");
		printf("</tr>\n");
		printf("<tr>\n");
		printf("<td align=left colspan=2 CLASS='optBoxItem'><select name='type'>\n");
		printf("<option value=%d %s>%s\n", NOTIFICATION_ALL, (notification_options == NOTIFICATION_ALL) ? "selected" : "",_("All notifications"));
		if(query_type != FIND_SERVICE) {
			printf("<option value=%d %s>%s\n", NOTIFICATION_SERVICE_ALL, (notification_options == NOTIFICATION_SERVICE_ALL) ? "selected" : "",_("All service notifications"));
			printf("<option value=%d %s>%s\n", NOTIFICATION_HOST_ALL, (notification_options == NOTIFICATION_HOST_ALL) ? "selected" : "",_("All host notifications"));
			}
		printf("<option value=%d %s>%s\n", NOTIFICATION_SERVICE_CUSTOM, (notification_options == NOTIFICATION_SERVICE_CUSTOM) ? "selected" : "",_("Service custom"));
		printf("<option value=%d %s>%s\n", NOTIFICATION_SERVICE_ACK, (notification_options == NOTIFICATION_SERVICE_ACK) ? "selected" : "",_("Service acknowledgements"));
		printf("<option value=%d %s>%s\n", NOTIFICATION_SERVICE_WARNING, (notification_options == NOTIFICATION_SERVICE_WARNING) ? "selected" : "",_("Service warning"));
		printf("<option value=%d %s>%s\n", NOTIFICATION_SERVICE_UNKNOWN, (notification_options == NOTIFICATION_SERVICE_UNKNOWN) ? "selected" : "",_("Service unknown"));
		printf("<option value=%d %s>%s\n", NOTIFICATION_SERVICE_CRITICAL, (notification_options == NOTIFICATION_SERVICE_CRITICAL) ? "selected" : "",_("Service critical"));
		printf("<option value=%d %s>%s\n", NOTIFICATION_SERVICE_RECOVERY, (notification_options == NOTIFICATION_SERVICE_RECOVERY) ? "selected" : "",_("Service recovery"));
		printf("<option value=%d %s>%s\n", NOTIFICATION_SERVICE_FLAP, (notification_options == NOTIFICATION_SERVICE_FLAP) ? "selected" : "",_("Service flapping"));
		if(query_type != FIND_SERVICE) {
			printf("<option value=%d %s>%s\n", NOTIFICATION_HOST_CUSTOM, (notification_options == NOTIFICATION_HOST_CUSTOM) ? "selected" : "",_("Host custom"));
			printf("<option value=%d %s>%s\n", NOTIFICATION_HOST_ACK, (notification_options == NOTIFICATION_HOST_ACK) ? "selected" : "",_("Host acknowledgements"));
			printf("<option value=%d %s>%s\n", NOTIFICATION_HOST_DOWN, (notification_options == NOTIFICATION_HOST_DOWN) ? "selected" : "",_("Host down"));
			printf("<option value=%d %s>%s\n", NOTIFICATION_HOST_UNREACHABLE, (notification_options == NOTIFICATION_HOST_UNREACHABLE) ? "selected" : "",_("Host unreachable"));
			printf("<option value=%d %s>%s\n", NOTIFICATION_HOST_RECOVERY, (notification_options == NOTIFICATION_HOST_RECOVERY) ? "selected" : "",_("Host recovery"));
			printf("<option value=%d %s>%s\n", NOTIFICATION_HOST_FLAP, (notification_options == NOTIFICATION_HOST_FLAP) ? "selected" : "",_("Host flapping"));
			}
		printf("</select></td>\n");
		printf("</tr>\n");
		printf("<tr>\n");
		printf("<td align=left CLASS='optBoxItem'>%s:</td>\n",_("Older Entries First"));
		printf("<td></td>\n");
		printf("</tr>\n");
		printf("<tr>\n");
		printf("<td align=left valign=bottom CLASS='optBoxItem'><input type='checkbox' name='oldestfirst' %s></td>", (use_lifo == FALSE) ? "checked" : "");
		printf("<td align=right CLASS='optBoxItem'><input type='submit' value='%s'></td>\n",_("Update"));
		printf("</tr>\n");
...
void display_notifications(void) {
	...
	if(use_lifo == TRUE) {
		...
		if(result != LIFO_OK) {
			if(result == LIFO_ERROR_MEMORY) {
				printf("<P><DIV CLASS='warningMessage'>%s</DIV></P>",_("Not enough memory to reverse log file - displaying notifications in natural order..."));
				}
			else if(result == LIFO_ERROR_FILE) {
				printf("<P><DIV CLASS='errorMessage'>%s</DIV></P>", log_file_to_use,_("Error: Cannot open log file '%s' for reading!"));
				return;
				}
			use_lifo = FALSE;
			}
	...
	printf("<tr>\n");
	printf("<th CLASS='notifications'>%s</th>\n",_("Host"));
	printf("<th CLASS='notifications'>%s</th>\n",_("Host"));
	printf("<th CLASS='notifications'>%s</th>\n",_("Type"));
	printf("<th CLASS='notifications'>%s</th>\n",_("Time"));
	printf("<th CLASS='notifications'>%s</th>\n",_("Contact"));
	printf("<th CLASS='notifications'>%s</th>\n",_("Notification Command"));
	printf("<th CLASS='notifications'>%s</th>\n",_("Information"));
	printf("</tr>\n");
	...
	if(total_notifications == 0) {
		printf("<P><DIV CLASS='errorMessage'>%s",_("No notifications have been recorded"));
		if(find_all == FALSE) {
			if(query_type == FIND_SERVICE)
				printf(" %s",_("for this service"));
			else if(query_type == FIND_CONTACT)
				printf(" %s",_("for this contact"));
			else
				printf(" %s",_("for this host"));
			}
