...
void display_network_outages(void) {
	...
	if(is_authorized_for_all_hosts(&current_authdata) == FALSE) {
		...
		printf("<P><DIV CLASS='errorMessage'>%s</DIV></P>\n",_("It appears as though you do not have permission to view information you requested..."));
	...
	printf("<TR>\n");
	printf("<TH CLASS='data'>%s</TH><TH CLASS='data'>%s</TH><TH CLASS='data'>%s</TH><TH CLASS='data'>%s</TH><TH CLASS='data'>%s</TH><TH CLASS='data'># %s</TH><TH CLASS='data'># %s</TH><TH CLASS='data'>%s</TH>\n",_("Severity"),_("Host"),_("State"),_("Notes"),_("State Duration"),_("Hosts Affected"),_("Services Affected"),_("Actions"));
	printf("</TR>\n");
	...
	if(total_entries == 0)
		printf("<DIV CLASS='itemTotalsTitle'>%d %s</DIV>\n", total_entries,_("Blocking Outages Displayed"));
