...
int main(void) {
	...
	if(display_header == TRUE) {
		...
		if(display_type == DISPLAY_HOSTS) {
			printf("<A HREF='%s?host=%s'>%s %s</A><BR />\n", STATUS_CGI, (show_all_hosts == TRUE) ? "all" : url_encode(host_name),_("View Status Detail For"), (show_all_hosts == TRUE) ? "All Hosts" : "This Host");
			...
			printf("service=%s'>%s</A><BR />\n", url_encode(svc_description),_("View Trends For This Service"));
#endif
		...
		printf("<tr>\n");
		printf("<td align=left CLASS='optBoxItem'>%s:</td>\n",_("State type options"));
		printf("</tr>\n");

		printf("<tr>\n");
		printf("<td align=left CLASS='optBoxItem'><select name='statetype'>\n");
		printf("<option value=%d %s>%s</option>\n", STATE_ALL, (state_options == STATE_ALL) ? "selected" : "",_("All state types"));
		printf("<option value=%d %s>%s</option>\n", STATE_SOFT, (state_options == STATE_SOFT) ? "selected" : "",_("Soft states"));
		printf("<option value=%d %s>%s</option>\n", STATE_HARD, (state_options == STATE_HARD) ? "selected" : "",_("Hard states"));
		printf("</select></td>\n");
		printf("</tr>\n");

		printf("<tr>\n");
		printf("<td align=left CLASS='optBoxItem'>%s ",_("History detail level for"));
		if(display_type == DISPLAY_HOSTS)
		...
		if(display_type == DISPLAY_HOSTS)
			printf("<option value=%d %s>%s</option>\n", HISTORY_ALL, (history_options == HISTORY_ALL) ? "selected" : "",_("All alerts"));
		printf("<option value=%d %s>%s</option>\n", HISTORY_SERVICE_ALL, (history_options == HISTORY_SERVICE_ALL) ? "selected" : "",_("All service alerts"));
		if(display_type == DISPLAY_HOSTS)
			printf("<option value=%d %s>%s</option>\n", HISTORY_HOST_ALL, (history_options == HISTORY_HOST_ALL) ? "selected" : "",_("All host alerts"));
		printf("<option value=%d %s>%s</option>\n", HISTORY_SERVICE_WARNING, (history_options == HISTORY_SERVICE_WARNING) ? "selected" : "",_("Service warning"));
		printf("<option value=%d %s>%s</option>\n", HISTORY_SERVICE_UNKNOWN, (history_options == HISTORY_SERVICE_UNKNOWN) ? "selected" : "",_("Service unknown"));
		printf("<option value=%d %s>%s</option>\n", HISTORY_SERVICE_CRITICAL, (history_options == HISTORY_SERVICE_CRITICAL) ? "selected" : "",_("Service critical"));
		printf("<option value=%d %s>%s</option>\n", HISTORY_SERVICE_RECOVERY, (history_options == HISTORY_SERVICE_RECOVERY) ? "selected" : "",_("Service recovery"));
		if(display_type == DISPLAY_HOSTS) {
			printf("<option value=%d %s>%s</option>\n", HISTORY_HOST_DOWN, (history_options == HISTORY_HOST_DOWN) ? "selected" : "",_("Host down"));
			printf("<option value=%d %s>%s</option>\n", HISTORY_HOST_UNREACHABLE, (history_options == HISTORY_HOST_UNREACHABLE) ? "selected" : "",_("Host unreachable"));
			printf("<option value=%d %s>%s</option>\n", HISTORY_HOST_RECOVERY, (history_options == HISTORY_HOST_RECOVERY) ? "selected" : "",_("Host recovery"));
			}
		printf("</select></td>\n");
		printf("</tr>\n");

		printf("<tr>\n");
		printf("<td align=left valign=bottom CLASS='optBoxItem'><input type='checkbox' name='noflapping' %s> %s</td>", (display_flapping_alerts == FALSE) ? "checked" : "",_("Hide Flapping Alerts"));
		printf("</tr>\n");
		printf("<tr>\n");
		printf("<td align=left valign=bottom CLASS='optBoxItem'><input type='checkbox' name='nodowntime' %s> %s</td>", (display_downtime_alerts == FALSE) ? "checked" : "",_("Hide Downtime Alerts"));
		printf("</tr>\n");

		printf("<tr>\n");
		printf("<td align=left valign=bottom CLASS='optBoxItem'><input type='checkbox' name='nosystem' %s> %s</td>", (display_system_messages == FALSE) ? "checked" : "",_("Hide Process Messages"));
		printf("</tr>\n");
		printf("<tr>\n");
		printf("<td align=left valign=bottom CLASS='optBoxItem'><input type='checkbox' name='oldestfirst' %s> %s</td>", (use_lifo == FALSE) ? "checked" : "",_("Older Entries First"));
		printf("</tr>\n");

		printf("<tr>\n");
		printf("<td align=left CLASS='optBoxItem'><input type='submit' value='%s'></td>\n",_("Update"));
		printf("</tr>\n");
...
void get_history(void) {
	...
	if(use_lifo == TRUE) {
		...
		if(result != LIFO_OK) {
			if(result == LIFO_ERROR_MEMORY) {
				printf("<P><DIV CLASS='warningMessage'>%s</DIV></P>\n",_("Not enough memory to reverse log file - displaying history in natural order..."));
				}
			else if(result == LIFO_ERROR_FILE) {
				printf("<HR><P><DIV CLASS='errorMessage'>%s</DIV></P><HR>", log_file_to_use,_("Error: Cannot open log file '%s' for reading!"));
				return;
				}
			use_lifo = FALSE;
			}
	...
	if(found_line == FALSE) {
		printf("<HR>\n");
		printf("<P><DIV CLASS='warningMessage'>%s ",_("No history information was found"));
		if(display_type == DISPLAY_HOSTS)
			printf("%s", (show_all_hosts == TRUE) ? "" : _("for this host"));
		else
			printf(_("for this service "));
