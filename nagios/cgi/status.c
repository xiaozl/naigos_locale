...
int main(void) {
	...
	if(display_header == TRUE) {
		...
		if(display_type == DISPLAY_HOSTS) {
			printf("<a href='%s?host=%s'>%s %s</a><br>\n", HISTORY_CGI, (show_all_hosts == TRUE) ? "all" : url_encode(host_name), _("View History For"),(show_all_hosts == TRUE) ? "all hosts" : "This Host");
			printf("<a href='%s?host=%s'>%s %s</a>\n", NOTIFICATIONS_CGI, (show_all_hosts == TRUE) ? "all" : url_encode(host_name), _("View Notifications For"),(show_all_hosts == TRUE) ? "All Hosts" : "This Host");
			if(show_all_hosts == FALSE)
				printf("<br /><a href='%s?host=all'>%s</a><br />\n", STATUS_CGI,_("View Service Status Detail For All Hosts"));
			else
				printf("<br /><a href='%s?hostgroup=all&style=hostdetail'>%s</a><br />\n", STATUS_CGI,_("View Host Status Detail For All Hosts"));
			}
			...
			if(show_all_servicegroups == FALSE) {
				...
				if(group_style_type == STYLE_OVERVIEW || group_style_type == STYLE_GRID || group_style_type == STYLE_SUMMARY)
					printf("<a href='%s?servicegroup=%s&style=detail'>%s</a><br>\n", STATUS_CGI, url_encode(servicegroup_name),_("View Host Status Detail For All Hosts"));
				if(group_style_type == STYLE_DETAIL || group_style_type == STYLE_GRID || group_style_type == STYLE_SUMMARY)
					printf("<a href='%s?servicegroup=%s&style=overview'>%s</a><br>\n", STATUS_CGI, url_encode(servicegroup_name),_("View Status Overview For This Service Group"));
				if(group_style_type == STYLE_DETAIL || group_style_type == STYLE_OVERVIEW || group_style_type == STYLE_GRID)
					printf("<a href='%s?servicegroup=%s&style=summary'>%s</a><br>\n", STATUS_CGI, url_encode(servicegroup_name),_("View Status Summary For This Service Group"));
				if(group_style_type == STYLE_DETAIL || group_style_type == STYLE_OVERVIEW || group_style_type == STYLE_SUMMARY)
					printf("<a href='%s?servicegroup=%s&style=grid'>%s</a><br>\n", STATUS_CGI, url_encode(servicegroup_name),_("View Service Status Grid For This Service Group"));

				if(group_style_type == STYLE_DETAIL)
					printf("<a href='%s?servicegroup=all&style=detail'>%s</a><br>\n", STATUS_CGI,_("View Service Status Detail For All Service Groups"));
				if(group_style_type == STYLE_OVERVIEW)
					printf("<a href='%s?servicegroup=all&style=overview'>%s</a><br>\n", STATUS_CGI,_("View Status Overview For All Service Groups"));
				if(group_style_type == STYLE_SUMMARY)
					printf("<a href='%s?servicegroup=all&style=summary'>%s</a><br>\n", STATUS_CGI,_("View Status Summary For All Service Groups"));
				if(group_style_type == STYLE_GRID)
					printf("<a href='%s?servicegroup=all&style=grid'>%s</a><br>\n", STATUS_CGI,_("View Status Summary For All Service Groups"));
				...
				if(group_style_type == STYLE_DETAIL || group_style_type == STYLE_OVERVIEW || group_style_type == STYLE_GRID)
					printf("<a href='%s?servicegroup=all&style=summary'>%s</a><br>\n", STATUS_CGI,_("View Status Summary For All Service Groups"));
				if(group_style_type == STYLE_DETAIL || group_style_type == STYLE_OVERVIEW || group_style_type == STYLE_SUMMARY)
					printf("<a href='%s?servicegroup=all&style=grid'>%s</a><br>\n", STATUS_CGI,_("View Service Status Grid For All Service Groups"));
				}
			...
			if(show_all_hostgroups == FALSE) {
				...
				if(group_style_type == STYLE_DETAIL)
					printf("<a href='%s?hostgroup=all&style=detail'>%s</a><br>\n", STATUS_CGI,_("View Service Status Detail For All Host Groups"));
				if(group_style_type == STYLE_HOST_DETAIL)
					printf("<a href='%s?hostgroup=all&style=hostdetail'>%s</a><br>\n", STATUS_CGI,_("View Host Status Detail For All Host Groups"));
				if(group_style_type == STYLE_OVERVIEW)
					printf("<a href='%s?hostgroup=all&style=overview'>%s</a><br>\n", STATUS_CGI,_("View Status Overview For All Host Groups"));
				if(group_style_type == STYLE_SUMMARY)
					printf("<a href='%s?hostgroup=all&style=summary'>%s</a><br>\n", STATUS_CGI,_("View Status Summary For All Host Groups"));
				if(group_style_type == STYLE_GRID)
					printf("<a href='%s?hostgroup=all&style=grid'>%s</a><br>\n", STATUS_CGI,_("View Status Grid For All Host Groups"));
				...
				if(group_style_type == STYLE_OVERVIEW || group_style_type == STYLE_SUMMARY || group_style_type == STYLE_GRID || group_style_type == STYLE_HOST_DETAIL)
					printf("<a href='%s?hostgroup=%s&style=detail'>%s</a><br>\n", STATUS_CGI, url_encode(hostgroup_name),_("View Service Status Detail For This Host Group"));
				if(group_style_type == STYLE_OVERVIEW || group_style_type == STYLE_DETAIL || group_style_type == STYLE_SUMMARY || group_style_type == STYLE_GRID)
					printf("<a href='%s?hostgroup=%s&style=hostdetail'>%s</a><br>\n", STATUS_CGI, url_encode(hostgroup_name),_("View Host Status Detail For This Host Group"));
				if(group_style_type == STYLE_DETAIL || group_style_type == STYLE_SUMMARY || group_style_type == STYLE_GRID || group_style_type == STYLE_HOST_DETAIL)
					printf("<a href='%s?hostgroup=%s&style=overview'>%s</a><br>\n", STATUS_CGI, url_encode(hostgroup_name),_("View Status Overview For This Host Group"));
				if(group_style_type == STYLE_OVERVIEW || group_style_type == STYLE_DETAIL || group_style_type == STYLE_GRID || group_style_type == STYLE_HOST_DETAIL)
					printf("<a href='%s?hostgroup=%s&style=summary'>%s</a><br>\n", STATUS_CGI, url_encode(hostgroup_name),_("View Status Summary For This Host Group"));
				if(group_style_type == STYLE_OVERVIEW || group_style_type == STYLE_DETAIL || group_style_type == STYLE_SUMMARY || group_style_type == STYLE_HOST_DETAIL)
					printf("<a href='%s?hostgroup=%s&style=grid'>%s</a><br>\n", STATUS_CGI, url_encode(hostgroup_name),_("View Status Grid For This Host Group"));
				}
			else {
				if(group_style_type == STYLE_OVERVIEW || group_style_type == STYLE_SUMMARY || group_style_type == STYLE_GRID || group_style_type == STYLE_HOST_DETAIL)
					printf("<a href='%s?hostgroup=all&style=detail'>%s</a><br>\n", STATUS_CGI,_("View Service Status Detail For All Host Groups"));
	...
	if(problem_hosts_unreachable > 0 && host_unreachable_sound != NULL)
		...
		sound = normal_sound;
	if(sound != NULL) {
		printf("<object type=\"audio/x-wav\" data=\"\/media%s%s\" height=\"0\" width=\"0\">", url_media_path, sound);
...
void show_service_status_totals(void) {
	...
	printf("<div class='serviceTotals'>%s</div>\n",_("Service Status Totals"));
...
void show_host_status_totals(void) {
	...
	printf("<div class='hostTotals'>%s</div>\n",_("Service Status Totals"));
...
void show_service_detail(void) {
	...
	printf("<div align='center' class='statusTitle'>%s ",_("Service Status Details For"));
	if(display_type == DISPLAY_HOSTS) {
		...
		if(show_all_servicegroups == TRUE)
			printf("%s",_("All Service Groups"));
		else
			printf("%s '%s'", _("Service Group"),url_encode(servicegroup_name));
		}
	else {
		if(show_all_hostgroups == TRUE)
			printf("%s",_("All Host Groups"));
		else
			printf("%s '%s'", _("Host Group"),hostgroup_name);
		}
	printf("</div>\n");
	...
	if(use_sort == TRUE) {
		...
		if(sort_option == SORT_HOSTNAME)
			...
			printf("%s",_("attempt number"));
		else if(sort_option == SORT_STATEDURATION)
	
			printf("%s",_("state duration"));
		printf("</b> (%s)\n", (sort_type == SORT_ASCENDING) ? "ascending" : "descending");
		printf("</div>\n");
		}

	if(service_filter != NULL)
		printf("<div align='center' class='statusSort'>%s \'%s\'</div>", _("Filtered By Services Matching"),service_filter);
	...
	*/
	...
	if(user_has_seen_something == FALSE) {
		...
		if(servicestatus_list != NULL) {
			printf("<P><div class='errorMessage'>%s</div></P>\n",_("It appears as though you do not have permission to view information for any of the services you requested..."));
			...
			printf("<p><div class='infoMessage'>%s<br><br>\n",_("There doesn't appear to be any service status information in the status log..."));
			printf("%s</div></p>\n",_("Make sure that Nagios is running and that you have specified the location of you status log correctly in the configuration files."));
			}
		}
	else {
...
//	printf("<P>\n");
	...
	printf("<div align='center' class='statusTitle'>%s ",_("Host Status Details For"));
	if(show_all_hostgroups == TRUE)
		printf("%s",_("All Host Groups"));
	else
		printf("%s '%s'", _("Host Group"),hostgroup_name);
	...
	if(use_sort == TRUE) {
		...
		if(sort_option == SORT_HOSTNAME)
			...
			printf("%s",_("host status"));
		else if(sort_option == SORT_HOSTURGENCY)
			printf("%s",_("host urgency"));
		else if(sort_option == SORT_LASTCHECKTIME)
			printf("%s",_("last check time"));
		else if(sort_option == SORT_CURRENTATTEMPT)
			printf("%s",_("attempt number"));
	...
	*/
	...
	printf("<th class='status'>%s&nbsp;<a href='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' border=0 ALT='Sort by host name (ascending)' TITLE='Sort by host name (ascending)'></a><a href='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' border=0 ALT='Sort by host name (descending)' TITLE='Sort by host name (descending)'></a></th>", _("Host"),temp_url, SORT_ASCENDING, SORT_HOSTNAME, url_images_path, UP_ARROW_ICON, temp_url, SORT_DESCENDING, SORT_HOSTNAME, url_images_path, DOWN_ARROW_ICON);

	printf("<th class='status'>%s&nbsp;<a href='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' border=0 ALT='Sort by host status (ascending)' TITLE='Sort by host status (ascending)'></a><a href='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' border=0 ALT='Sort by host status (descending)' TITLE='Sort by host status (descending)'></a></th>", _("Status"),temp_url, SORT_ASCENDING, SORT_HOSTSTATUS, url_images_path, UP_ARROW_ICON, temp_url, SORT_DESCENDING, SORT_HOSTSTATUS, url_images_path, DOWN_ARROW_ICON);

	printf("<th class='status'>%s&nbsp;<a href='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' border=0 ALT='Sort by last check time (ascending)' TITLE='Sort by last check time (ascending)'></a><a href='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' border=0 ALT='Sort by last check time (descending)' TITLE='Sort by last check time (descending)'></a></th>", _("Last Check"),temp_url, SORT_ASCENDING, SORT_LASTCHECKTIME, url_images_path, UP_ARROW_ICON, temp_url, SORT_DESCENDING, SORT_LASTCHECKTIME, url_images_path, DOWN_ARROW_ICON);

	printf("<th class='status'>%s&nbsp;<a href='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' border=0 ALT='Sort by state duration (ascending)' TITLE='Sort by state duration (ascending)'></a><a href='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' border=0 ALT='Sort by state duration time (descending)' TITLE='Sort by state duration time (descending)'></a></th>", _("Duration"),temp_url, SORT_ASCENDING, SORT_STATEDURATION, url_images_path, UP_ARROW_ICON, temp_url, SORT_DESCENDING, SORT_STATEDURATION, url_images_path, DOWN_ARROW_ICON);

	printf("<th class='status'>%s</th>\n",_("Status Information"));
	printf("</tr>\n");
	...
	if(user_has_seen_something == FALSE) {
		...
		if(hoststatus_list != NULL) {
			printf("<P><div class='errorMessage'>%s</div></P>\n",_("It appears as though you do not have permission to view information for any of the hosts you requested..."));
			printf("<P><div class='errorDescription'>%s</div></P>\n",_("If you believe this is an error, check the HTTP server authentication requirements for accessing this CGI<br>and check the authorization options in your CGI configuration file."));
			
			}
		else {
			printf("<P><div class='infoMessage'>%s<br><br>\n",_("There doesn't appear to be any host status information in the status log..."));
			printf("%s</div></P>\n",_("Make sure that Nagios is running and that you have specified the location of you status log correctly in the configuration files."));
			}
		}
...
void show_servicegroup_overviews(void) {
	...
	printf("<div align='center' class='statusTitle'>%s ",_("Service Overview For"));
	if(show_all_servicegroups == TRUE)
		printf("%s",_("All Service Groups"));
	else
		printf("%s '%s'", _("Service Group"),servicegroup_name);
	printf("</div>\n");
	...
	/* else display overview for just a specific servicegroup */
		...
		if(temp_servicegroup != NULL) {
			...
			printf("<div class='errorMessage'>%s '%s' %s...</div>",_("Sorry, but service group"), servicegroup_name,_("doesn't seem to exist"));
	...
	if(user_has_seen_something == FALSE && servicegroup_error == FALSE) {
		...
		if(servicegroup_list != NULL) {
			printf("<div class='errorMessage'>%s</div>\n",_("It appears as though you do not have permission to view information for any of the hosts you requested..."));
			printf("<div class='errorDescription'>%s</div>\n",_("If you believe this is an error, check the HTTP server authentication requirements for accessing this CGI<br>and check the authorization options in your CGI configuration file."));
			
			}
		else {
			printf("<div class='errorMessage'>%s</div>\n",_("There are no service groups defined."));
			}
...
void show_servicegroup_overview(servicegroup *temp_servicegroup) {
	...
	printf("<tr>\n");
	printf("<th class='status'>%s</th><th class='status'>%s</th><th class='status'>%s</th><th class='status'>Actions</th>\n",_("Host"),_("Status"),_("Services"));
	printf("</tr>\n");
...
void show_servicegroup_summaries(void) {
	...
	printf("<div align='center' class='statusTitle'>%s ",_("Status Summary For"));
	if(show_all_servicegroups == TRUE)
		printf("%s",_("All Service Groups"));
	else
		printf("%s '%s'", _("Service Group"),servicegroup_name);
	...
	printf("<tr>\n");
	printf("<th class='status'>%s</th><th class='status'>%s</th><th class='status'>%s</th>\n",_("Service Group"),_("Host Status Summary"),_("Service Status Summary"));
	printf("</tr>\n");
	...
	/* we couldn't find the servicegroup */
		...
		printf("<P><div align='center'>\n");
		printf("<div class='errorMessage'>%s '%s' %s...</div>\n", _("Sorry, but servicegroup"),servicegroup_name,_("doesn't seem to exist"));
		printf("</div></P>\n");
		}
...
void show_hostgroup_overviews(void) {
	...
	/* else display overview for just a specific hostgroup */
		...
		if(temp_hostgroup != NULL) {
			...
			printf("<div class='errorMessage'>%s '%s' %s...</div>", _("Sorry, but host group"),hostgroup_name,_("doesn't seem to exist"));
	...
	if(user_has_seen_something == FALSE && hostgroup_error == FALSE) {
		...
		if(hoststatus_list != NULL) {
			...
			printf("<div class='infoMessage'>%s<br><br>\n",_("There doesn't appear to be any host status information in the status log..."));
			printf("%s</div>\n",_("Make sure that Nagios is running and that you have specified the location of you status log correctly in the configuration files."));
			}
...
void show_hostgroup_overview(hostgroup *hstgrp) {
	...
	printf("<tr>\n");
	printf("<th class='status'>%s</th><th class='status'>%s</th><th class='status'>%s</th><th class='status'>%s</th>\n",_("Host"),_("Status"),_("Status"),_("Actions"));
	printf("</tr>\n");
...
void show_servicegroup_hostgroup_member_service_status_totals(char *host_name, void *data) {
	...
	if((total_ok + total_warning + total_unknown + total_critical + total_pending) == 0)
		printf("%s",_("No matching services"));
...
void show_hostgroup_summaries(void) {
	...
	printf("<tr>\n");
	printf("<th class='status'>%s</th><th class='status'>%s</th><th class='status'>%s</th>\n",_("Host Group"),_("Host Status Summary"),_("Service Status Summary"));
	printf("</tr>\n");
	...
	/* we couldn't find the hostgroup */
		...
		printf("<P><div align='center'>\n");
		printf("<div class='errorMessage'>%s '%s' %s...</div>\n",_("Sorry, but hostgroup") ,hostgroup_name,_("doesn't seem to exist"));
		printf("</div></P>\n");
		}
...
void show_hostgroup_grids(void) {
	...
	printf("<div align='center' class='statusTitle'>%s ",_("Status Grid For"));
	if(show_all_hostgroups == TRUE)
		printf("%s",_("All Host Groups"));
	else
		printf("%s '%s'", _("Host Group"),hostgroup_name);
...
void show_filters(void) {
	...
	if(host_properties != 0L || service_properties != 0L || host_status_types != all_host_status_types || service_status_types != all_service_status_types) {
		...
		printf("<tr><td colspan=2 valign=top align=left class='filterTitle'>%s:</td></tr>",_("Display Filters"));
		printf("<tr><td valign=top align=left class='filterName'>%s:</td>",_("Host Status Types"));
		printf("<td valign=top align=left class='filterValue'>");
		if(host_status_types == all_host_status_types)
			printf("%s",_("All"));
		else if(host_status_types == all_host_problems)
			printf("%s",_("All problems"));
		else {
			found = 0;
			...
			if(host_status_types & HOST_UP) {
				printf("%s %s", (found == 1) ? " |" : "",_("Up"));
				found = 1;
				}
			if(host_status_types & HOST_DOWN) {
				printf("%s %s", (found == 1) ? " |" : "",_("Down"));
				found = 1;
				}
			if(host_status_types & HOST_UNREACHABLE)
				printf("%s %s", (found == 1) ? " |" : "",_("Unreachable"));
			}
		printf("</td></tr>");
		printf("<tr><td valign=top align=left class='filterName'>%s:</td>",_("Host Properties"));
		printf("<td valign=top align=left class='filterValue'>");
		if(host_properties == 0)
			printf("%s",_("Any"));
		else {
			...
			if(host_properties & HOST_SCHEDULED_DOWNTIME) {
				printf(" %s",_("In Scheduled Downtime"));
				found = 1;
				}
			if(host_properties & HOST_NO_SCHEDULED_DOWNTIME) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Not In Scheduled Downtime"));
				found = 1;
				}
			if(host_properties & HOST_STATE_ACKNOWLEDGED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Has Been Acknowledged"));
				found = 1;
				}
			if(host_properties & HOST_STATE_UNACKNOWLEDGED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Has Not Been Acknowledged"));
				found = 1;
				}
			...
			if(host_properties & HOST_CHECKS_ENABLED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Checks Enabled"));
				found = 1;
				}
			if(host_properties & HOST_EVENT_HANDLER_DISABLED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Event Handler Disabled"));
				found = 1;
				}
			if(host_properties & HOST_EVENT_HANDLER_ENABLED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Event Handler Enabled"));
				found = 1;
				}
			if(host_properties & HOST_FLAP_DETECTION_DISABLED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Flap Detection Disabled"));
				found = 1;
				}
			if(host_properties & HOST_FLAP_DETECTION_ENABLED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Flap Detection Enabled"));
				found = 1;
				}
			if(host_properties & HOST_IS_FLAPPING) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Is Flapping"));
				found = 1;
				}
			if(host_properties & HOST_IS_NOT_FLAPPING) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Is Not Flapping"));
				found = 1;
				}
			...
			if(host_properties & HOST_NOTIFICATIONS_ENABLED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Notifications Enabled"));
				found = 1;
				}
			if(host_properties & HOST_PASSIVE_CHECKS_DISABLED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Passive Checks Disabled"));
				found = 1;
				}
			if(host_properties & HOST_PASSIVE_CHECKS_ENABLED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Passive Checks Enabled"));
				found = 1;
				}
			if(host_properties & HOST_PASSIVE_CHECK) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Passive Checks"));
				found = 1;
				}
			if(host_properties & HOST_ACTIVE_CHECK) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Active Checks"));
				found = 1;
				}
			if(host_properties & HOST_HARD_STATE) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("In Hard State"));
				found = 1;
				}
			if(host_properties & HOST_SOFT_STATE) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("In Soft State"));
				found = 1;
				}
			}
		printf("</td>");
		...
		printf("<tr><td valign=top align=left class='filterName'>%s:</td>",_("In Soft State"));
		printf("<td valign=top align=left class='filterValue'>");
		if(service_status_types == all_service_status_types)
			printf("%s",_("All"));
			...
			if(service_status_types & SERVICE_OK) {
				printf("%s %s", (found == 1) ? " |" : "",_("Ok"));
				found = 1;
				}
			if(service_status_types & SERVICE_UNKNOWN) {
				printf("%s %s", (found == 1) ? " |" : "",_("Unknown"));
				found = 1;
				}
			if(service_status_types & SERVICE_WARNING) {
				printf("%s %s", (found == 1) ? " |" : "",_("Warning"));
				found = 1;
				}
			if(service_status_types & SERVICE_CRITICAL) {
				printf("%s %s", (found == 1) ? " |" : "",_("Critical"));
				found = 1;
				}
			}
		printf("</td></tr>");
		printf("<tr><td valign=top align=left class='filterName'>%s:</td>",_("Service Properties"));
		printf("<td valign=top align=left class='filterValue'>");
		if(service_properties == 0)
			printf("%s",_("Any"));
		else {
			...
			if(service_properties & SERVICE_CHECKS_DISABLED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Active Checks Disabled"));
				found = 1;
				}
			if(service_properties & SERVICE_CHECKS_ENABLED) {
				printf("%s %s", (found == 1) ? " &amp;" : "",_("Active Checks Enabled"));
				found = 1;
				}
