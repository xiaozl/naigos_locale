...
int main(void) {
	...
	if(display_header == TRUE) {
		...
		if((display_type == DISPLAY_HOST_INFO && temp_host != NULL) || (display_type == DISPLAY_SERVICE_INFO && temp_host != NULL && temp_service != NULL) || (display_type == DISPLAY_HOSTGROUP_INFO && temp_hostgroup != NULL) || (display_type == DISPLAY_SERVICEGROUP_INFO && temp_servicegroup != NULL)) {
			...
			if(display_type == DISPLAY_SERVICE_INFO)
				printf("<A HREF='%s?type=%d&host=%s'>%s</A><br>\n", EXTINFO_CGI, DISPLAY_HOST_INFO, url_encode(host_name),_("View Information For This Host"));
			if(display_type == DISPLAY_SERVICE_INFO || display_type == DISPLAY_HOST_INFO)
				printf("<A HREF='%s?host=%s'>%s</A><BR>\n", STATUS_CGI, url_encode(host_name),_("View Status Detail For This Host"));
			if(display_type == DISPLAY_HOST_INFO) {
				printf("<A HREF='%s?host=%s'>%s</A><BR>\n", HISTORY_CGI, url_encode(host_name),_("View Alert History For This Host"));
#ifdef USE_TRENDS
				printf("<A HREF='%s?host=%s'>%s</A><BR>\n", TRENDS_CGI, url_encode(host_name),_("View Trends For This Host"));
#endif
#ifdef USE_HISTOGRAM
				printf("<A HREF='%s?host=%s'>%s</A><BR>\n", HISTOGRAM_CGI, url_encode(host_name),_("View Alert Histogram For This Host"));
#endif
				printf("<A HREF='%s?host=%s&show_log_entries'>%s</A><BR>\n", AVAIL_CGI, url_encode(host_name),_("View Availability Report For This Host"));
				printf("<A HREF='%s?host=%s'>%s</A>\n", NOTIFICATIONS_CGI, url_encode(host_name),_("View Notifications For This Host"));
				printf("<BR><A HREF='manage.php?host=%s'>%s</A>\n",url_encode(host_name),_("Manage This Host"));
				}
			else if(display_type == DISPLAY_SERVICE_INFO) {
				printf("<A HREF='%s?host=%s&", HISTORY_CGI, url_encode(host_name));
				printf("service=%s'>%s</A><BR>\n", url_encode(service_desc),_("View Alert History For This Service"));
#ifdef USE_TRENDS
				printf("<A HREF='%s?host=%s&", TRENDS_CGI, url_encode(host_name));
				printf("service=%s'>%s</A><BR>\n", url_encode(service_desc),_("View Trends For This Service"));
#endif
#ifdef USE_HISTOGRAM
				printf("<A HREF='%s?host=%s&", HISTOGRAM_CGI, url_encode(host_name));
				printf("service=%s'>%s</A><BR>\n", url_encode(service_desc),_("View Alert Histogram For This Service"));
#endif
				printf("<A HREF='%s?host=%s&", AVAIL_CGI, url_encode(host_name));
				printf("service=%s&show_log_entries'>%s</A><BR>\n", url_encode(service_desc),_("View Availability Report For This Service"));
				printf("<A HREF='%s?host=%s&", NOTIFICATIONS_CGI, url_encode(host_name));
				printf("service=%s'>%s</A>\n", url_encode(service_desc),_("View Notifications For This Service"));
				}
			else if(display_type == DISPLAY_HOSTGROUP_INFO) {
				printf("<A HREF='%s?hostgroup=%s&style=detail'>%s</A><BR>\n", STATUS_CGI, url_encode(hostgroup_name),_("View Status Detail For This Hostgroup"));
				printf("<A HREF='%s?hostgroup=%s&style=overview'>%s</A><BR>\n", STATUS_CGI, url_encode(hostgroup_name),_("View Status Overview For This Hostgroup"));
				printf("<A HREF='%s?hostgroup=%s&style=grid'>%s</A><BR>\n", STATUS_CGI, url_encode(hostgroup_name),_("View Status Grid For This Hostgroup"));
				printf("<A HREF='%s?hostgroup=%s'>%s</A><BR>\n", AVAIL_CGI, url_encode(hostgroup_name),_("View Availability For This Hostgroup"));
				}
			else if(display_type == DISPLAY_SERVICEGROUP_INFO) {
				printf("<A HREF='%s?servicegroup=%s&style=detail'>%s</A><BR>\n", STATUS_CGI, url_encode(servicegroup_name),_("View Status Detail For This Servicegroup"));
				printf("<A HREF='%s?servicegroup=%s&style=overview'>%s</A><BR>\n", STATUS_CGI, url_encode(servicegroup_name),_("View Status Overview For This Servicegroup"));
				printf("<A HREF='%s?servicegroup=%s&style=grid'>%s</A><BR>\n", STATUS_CGI, url_encode(servicegroup_name),_("View Status Grid For This Servicegroup"));
				printf("<A HREF='%s?servicegroup=%s'>%s</A><BR>\n", AVAIL_CGI, url_encode(servicegroup_name),_("View Availability For This Servicegroup"));
				}
			printf("<BR><a href='javascript:window.print()' target='_self'>%s</a>",_("View Availability For This Servicegroup"));
			printf("</TD></TR>\n");
			printf("</TABLE>\n");
			}
		...
		if((display_type == DISPLAY_HOST_INFO && temp_host != NULL) || (display_type == DISPLAY_SERVICE_INFO && temp_host != NULL && temp_service != NULL) || (display_type == DISPLAY_HOSTGROUP_INFO && temp_hostgroup != NULL) || (display_type == DISPLAY_SERVICEGROUP_INFO && temp_servicegroup != NULL)) {
			...
			if(display_type == DISPLAY_HOST_INFO) {
				...
				printf("<DIV CLASS='data'>%s</DIV><DIV CLASS='dataTitle'>",_("Member of"));
			...
			if(display_type == DISPLAY_SERVICE_INFO) {
				printf("<DIV CLASS='data'>%s</DIV><DIV CLASS='dataTitle'>%s</DIV><DIV CLASS='data'>%s</DIV>\n", _("Service"),service_desc,_("On Host"));
				printf("<DIV CLASS='dataTitle'>%s</DIV>\n", temp_host->alias);
		...
		if(display_type == DISPLAY_HOST_INFO && temp_host != NULL) {
			...
			if(temp_host->action_url != NULL && strcmp(temp_host->action_url, "")) {
				...
				printf("<BR CLEAR=ALL><FONT SIZE=-1><I>%s</I></FONT><BR CLEAR=ALL><BR CLEAR=ALL>\n",_("Extra Actions"));
				printf("</TD></TR>\n");
				}
...
void show_process_info(void) {
	...
	/* program version */
	printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%s</TD></TR>\n", PROGRAM_VERSION,_("Program Version"));

	/* program start time */
	get_time_string(&program_start, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
	printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%s</TD></TR>\n", date_time,_("Program Start Time"));
	...
	printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%s</TD></TR>\n",_("Program Start Time"), run_time_string);

	/* last external check */
	get_time_string(&last_command_check, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
	printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Last External Command Check"),(last_command_check == (time_t)0) ? "N/A" : date_time);

	/* last log file rotation */
	get_time_string(&last_log_rotation, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
	printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Last Log File Rotation"),(last_log_rotation == (time_t)0) ? "N/A" : date_time);

	/* PID */
	printf("<TR><TD CLASS='dataVar'>Nagios PID</TD><TD CLASS='dataVal'>%d</TD></TR>\n", nagios_pid);

	/* notifications enabled */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'><DIV CLASS='notifications%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Notifications Enabled"),(enable_notifications == TRUE) ? "ENABLED" : "DISABLED", (enable_notifications == TRUE) ? "YES" : "NO");

	/* service check execution enabled */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'><DIV CLASS='checks%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Service Checks Being Executed"),(execute_service_checks == TRUE) ? "ENABLED" : "DISABLED", (execute_service_checks == TRUE) ? "YES" : "NO");

	/* passive service check acceptance */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'><DIV CLASS='checks%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Passive Service Checks Being Accepted"),(accept_passive_service_checks == TRUE) ? "ENABLED" : "DISABLED", (accept_passive_service_checks == TRUE) ? "YES" : "NO");

	/* host check execution enabled */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'><DIV CLASS='checks%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Host Checks Being Executed"),(execute_host_checks == TRUE) ? "ENABLED" : "DISABLED", (execute_host_checks == TRUE) ? "YES" : "NO");

	/* passive host check acceptance */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'><DIV CLASS='checks%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Passive Host Checks Being Accepted"),(accept_passive_host_checks == TRUE) ? "ENABLED" : "DISABLED", (accept_passive_host_checks == TRUE) ? "YES" : "NO");

	/* event handlers enabled */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Event Handlers Enabled"),(enable_event_handlers == TRUE) ? "Yes" : "No");

	/* obsessing over services */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Obsessing Over Services"),(obsess_over_services == TRUE) ? "Yes" : "No");

	/* obsessing over hosts */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Obsessing Over Hosts"),(obsess_over_hosts == TRUE) ? "Yes" : "No");

	/* flap detection enabled */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Flap Detection Enabled"),(enable_flap_detection == TRUE) ? "Yes" : "No");

#ifdef PREDICT_FAILURES
	/* failure prediction enabled */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Failure Prediction Enabled"),(enable_failure_prediction == TRUE) ? "Yes" : "No");
#endif

	/* process performance data */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Performance Data Being Processed"),(process_performance_data == TRUE) ? "Yes" : "No");

#ifdef USE_OLDCRUD
	/* daemon mode */
	printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Running As A Daemon"),(daemon_mode == TRUE) ? "Yes" : "No");
#endif
	...
	printf("<DIV CLASS='commandTitle'>%s</DIV>\n",_("Process Commands"));
	...
	if(nagios_process_state == STATE_OK) {
		...
		printf("<DIV ALIGN=CENTER CLASS='infoMessage'>%s\n",_("It appears as though Nagios is not running, so commands are temporarily unavailable..."));
		if(!strcmp(nagios_check_command, "")) {
			printf("<BR><BR>\n");
			printf("%s\n",_("Hint: It looks as though you have not defined a command for checking the process state by supplying a value for the <b>nagios_check_command</b> option in the CGI configuration file.<BR>"));
			printf("%s\n",_("Read the documentation for more information on checking the status of the Nagios process in the CGIs."));
			}
		printf("</DIV>\n");
		}
...
void show_host_info(void) {
	...
	if(is_authorized_for_host(temp_host, &current_authdata) == FALSE) {
		...
		printf("<P><DIV CLASS='errorMessage'>%s</DIV></P>\n",_("It appears as though you do not have permission to view information for this host..."));
		printf("<P><DIV CLASS='errorDescription'>%s</DIV></P>\n",_("If you believe this is an error, check the HTTP server authentication requirements for accessing this CGI<br>and check the authorization options in your CGI configuration file."));
	...
	if(temp_host == NULL) {
		printf("<P><DIV CLASS='errorMessage'>%s</DIV></P>>",_("Error: Host Not Found!"));
		return;
		}
	if(temp_hoststatus == NULL) {
		printf("<P><DIV CLASS='errorMessage'>%s</DIV></P",_("Error: Host Status Information Not Found!"));
		return;
		}
	...
	printf("<DIV CLASS='dataTitle'>%s</DIV>\n",_("Host State Information"));

	if(temp_hoststatus->has_been_checked == FALSE)
		printf("<P><DIV ALIGN=CENTER>%s</DIV></P>\n",_("This host has not yet been checked, so status information is not available."));
		...
		printf("<TR><TD CLASS='dataVar'>%s:</td><td CLASS='dataVal'><DIV CLASS='%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV>&nbsp;(for %s)%s</td></tr>\n", _("Host Status"),bg_class, state_string, state_duration, (temp_hoststatus->problem_has_been_acknowledged == TRUE) ? "&nbsp;&nbsp;(Has been acknowledged)" : "");

		printf("<TR><TD CLASS='dataVar' VALIGN='top'>%s:</td><td CLASS='dataVal'>%s", _("Status Information"),(temp_hoststatus->plugin_output == NULL) ? "" : html_encode(temp_hoststatus->plugin_output, TRUE));
		...
		printf("<TR><TD CLASS='dataVar' VALIGN='top'>%s:</td><td CLASS='dataVal'>%s</td></tr>\n", _("Performance Data"),(temp_hoststatus->perf_data == NULL) ? "" : html_encode(temp_hoststatus->perf_data, TRUE));

		printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%d/%d", _("Current Attempt"),temp_hoststatus->current_attempt, temp_hoststatus->max_attempts);
		...
		printf("<TR><TD CLASS='dataVar'>%s:</td><td CLASS='dataVal'>%s</td></tr>\n", _("Last Check Time"),date_time);

		printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Check Type"),(temp_hoststatus->check_type == HOST_CHECK_ACTIVE) ? "ACTIVE" : "PASSIVE");

		printf("<TR><TD CLASS='dataVar' NOWRAP>%s:</TD><TD CLASS='dataVal'>",_("Check Latency / Duration"));
		if(temp_hoststatus->check_type == HOST_CHECK_ACTIVE)
			printf("%.3f", temp_hoststatus->latency);
		else
			printf("N/A");
		printf("&nbsp;/&nbsp;%.3f %s", temp_hoststatus->execution_time,_("seconds"));
		printf("</TD></TR>\n");

		get_time_string(&temp_hoststatus->next_check, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
		printf("<TR><TD CLASS='dataVar'>%s:&nbsp;&nbsp;</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Next Scheduled Active Check"),(temp_hoststatus->checks_enabled && temp_hoststatus->next_check != (time_t)0 && temp_hoststatus->should_be_scheduled == TRUE) ? date_time : "N/A");

		get_time_string(&temp_hoststatus->last_state_change, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
		printf("<TR><TD CLASS='dataVar'>%s:</td><td CLASS='dataVal'>%s</td></tr>\n", _("Last State Change"),(temp_hoststatus->last_state_change == (time_t)0) ? "N/A" : date_time);

		get_time_string(&temp_hoststatus->last_notification, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
		printf("<TR><TD CLASS='dataVar'>%s:</td><td CLASS='dataVal'>%s&nbsp;(notification %d)</td></tr>\n", _("Last Notification"),(temp_hoststatus->last_notification == (time_t)0) ? "N/A" : date_time, temp_hoststatus->current_notification_number);

		printf("<TR><TD CLASS='dataVar'>%s?</td><td CLASS='dataVal'>",_("Is This Host Flapping"));
		...
		printf("<TR><TD CLASS='dataVar'>%s?</td><td CLASS='dataVal'><DIV CLASS='downtime%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></td></tr>\n", _("In Scheduled Downtime"),(temp_hoststatus->scheduled_downtime_depth > 0) ? "ACTIVE" : "INACTIVE", (temp_hoststatus->scheduled_downtime_depth > 0) ? "YES" : "NO");
		...
		printf("<TR><TD CLASS='dataVar'>%s:</td><td CLASS='dataVal'>%s&nbsp;&nbsp;(%s ago)</td></tr>\n", _("Last Update"),(temp_hoststatus->last_update == (time_t)0) ? "N/A" : date_time, status_age);
		...
		printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'><DIV CLASS='checks%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Active Checks"),(temp_hoststatus->checks_enabled == TRUE) ? "ENABLED" : "DISABLED", (temp_hoststatus->checks_enabled == TRUE) ? "ENABLED" : "DISABLED");

		printf("<TR><TD CLASS='dataVar'>%s:</TD><td CLASS='dataVal'><DIV CLASS='checks%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Passive Checks"),(temp_hoststatus->accept_passive_host_checks == TRUE) ? "ENABLED" : "DISABLED", (temp_hoststatus->accept_passive_host_checks) ? "ENABLED" : "DISABLED");

		printf("<TR><TD CLASS='dataVar'>%s:</TD><td CLASS='dataVal'><DIV CLASS='checks%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Obsessing"),(temp_hoststatus->obsess_over_host == TRUE) ? "ENABLED" : "DISABLED", (temp_hoststatus->obsess_over_host) ? "ENABLED" : "DISABLED");

		printf("<TR><TD CLASS='dataVar'>%s:</td><td CLASS='dataVal'><DIV CLASS='notifications%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></td></tr>\n", _("Notifications"),(temp_hoststatus->notifications_enabled) ? "ENABLED" : "DISABLED", (temp_hoststatus->notifications_enabled) ? "ENABLED" : "DISABLED");

		printf("<TR><TD CLASS='dataVar'>%s:</td><td CLASS='dataVal'><DIV CLASS='eventhandlers%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></td></tr>\n", _("Event Handler"),(temp_hoststatus->event_handler_enabled) ? "ENABLED" : "DISABLED", (temp_hoststatus->event_handler_enabled) ? "ENABLED" : "DISABLED");

		printf("<TR><TD CLASS='dataVar'>%s:</td><td CLASS='dataVal'><DIV CLASS='flapdetection%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></td></tr>\n", _("Flap Detection"),(temp_hoststatus->flap_detection_enabled == TRUE) ? "ENABLED" : "DISABLED", (temp_hoststatus->flap_detection_enabled == TRUE) ? "ENABLED" : "DISABLED");
	...
	if(nagios_process_state == STATE_OK && is_authorized_for_read_only(&current_authdata) == FALSE) {
		...
		printf("<DIV ALIGN=CENTER CLASS='infoMessage'>%s<br>\n",_("It appears as though Nagios is not running, so commands are temporarily unavailable..."));
		printf(" <a href='%s?type=%d'>%s</a> %s</DIV>\n",EXTINFO_CGI, DISPLAY_PROCESS_INFO, _("Click"),_("view Nagios process information"));
		}
	printf("</TD></TR></TABLE>\n");
...
void show_service_info(void) {
	...
	if(is_authorized_for_service(temp_service, &current_authdata) == FALSE) {
		...
		printf("<P><DIV CLASS='errorMessage'>%s</DIV></P>\n",_("It appears as though you do not have permission to view information for this service..."));
		printf("<P><DIV CLASS='errorDescription'>%s</DIV></P>\n",_("If you believe this is an error, check the HTTP server authentication requirements for accessing this CGI<br>and check the authorization options in your CGI configuration file."));
	...
	if(temp_service == NULL) {
		printf("<P><DIV CLASS='errorMessage'>%s</DIV></P>",_("Error: Service Not Found!"));
		return;
		}
	if(temp_svcstatus == NULL) {
		printf("<P><DIV CLASS='errorMessage'>%s</DIV></P>",_("Error: Service Status Not Found!"));
		return;
		}
	...
	printf("<DIV CLASS='dataTitle'>%s</DIV>\n",_("Service State Information"));

	if(temp_svcstatus->has_been_checked == FALSE)
		printf("<P><DIV ALIGN=CENTER>%s</DIV></P>\n",_("This service has not yet been checked, so status information is not available."));
		...
		printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'><DIV CLASS='%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV>&nbsp;(for %s)%s</TD></TR>\n", _("Current Status"),bg_class, state_string, state_duration, (temp_svcstatus->problem_has_been_acknowledged == TRUE) ? "&nbsp;&nbsp;(Has been acknowledged)" : "");

		printf("<TR><TD CLASS='dataVar' VALIGN='top'>%s:</TD><TD CLASS='dataVal'>%s", _("Status Information"),(temp_svcstatus->plugin_output == NULL) ? "" : html_encode(temp_svcstatus->plugin_output, TRUE));
		if(enable_splunk_integration == TRUE) {
			printf("&nbsp;&nbsp;");
		...
		printf("<TR><TD CLASS='dataVar' VALIGN='top'>%s:</td><td CLASS='dataVal'>%s</td></tr>\n",_("Performance Data"), (temp_svcstatus->perf_data == NULL) ? "" : html_encode(temp_svcstatus->perf_data, TRUE));

		printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%d/%d", _("Current Attempt"),temp_svcstatus->current_attempt, temp_svcstatus->max_attempts);
		...
		printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Check Type"),(temp_svcstatus->check_type == SERVICE_CHECK_ACTIVE) ? "ACTIVE" : "PASSIVE");
		...
		if(temp_svcstatus->check_type == SERVICE_CHECK_ACTIVE)
			...
			printf("N/A");
		printf("&nbsp;/&nbsp;%.3f %s", temp_svcstatus->execution_time,_("seconds"));
		printf("</TD></TR>\n");

		get_time_string(&temp_svcstatus->next_check, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
		printf("<TR><TD CLASS='dataVar'>%s:&nbsp;&nbsp;</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Next Scheduled Check"),(temp_svcstatus->checks_enabled && temp_svcstatus->next_check != (time_t)0 && temp_svcstatus->should_be_scheduled == TRUE) ? date_time : "N/A");

		get_time_string(&temp_svcstatus->last_state_change, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
		printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%s</TD></TR>\n", _("Last State Change"),(temp_svcstatus->last_state_change == (time_t)0) ? "N/A" : date_time);

		get_time_string(&temp_svcstatus->last_notification, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
		printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%s&nbsp;(notification %d)</TD></TR>\n", _("Last Notification"),(temp_svcstatus->last_notification == (time_t)0) ? "N/A" : date_time, temp_svcstatus->current_notification_number);

		printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'>",_("Is This Service Flapping"));
		...
		printf("<TR><TD CLASS='dataVar'>%s?</TD><TD CLASS='dataVal'><DIV CLASS='downtime%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("In Scheduled Downtime"),(temp_svcstatus->scheduled_downtime_depth > 0) ? "ACTIVE" : "INACTIVE", (temp_svcstatus->scheduled_downtime_depth > 0) ? "YES" : "NO");
		...
		printf("<TR><TD CLASS='dataVar'>%s:</TD><TD CLASS='dataVal'>%s&nbsp;&nbsp;(%s ago)</TD></TR>\n", _("Last Update"),(temp_svcstatus->last_update == (time_t)0) ? "N/A" : date_time, status_age);
		...
		printf("<TR><TD CLASS='dataVar'>%s:</TD><td CLASS='dataVal'><DIV CLASS='checks%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Active Checks"),(temp_svcstatus->checks_enabled) ? "ENABLED" : "DISABLED", (temp_svcstatus->checks_enabled) ? "ENABLED" : "DISABLED");

		printf("<TR><TD CLASS='dataVar'>%s:</TD><td CLASS='dataVal'><DIV CLASS='checks%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Passive Checks"), (temp_svcstatus->accept_passive_service_checks == TRUE) ? "ENABLED" : "DISABLED", (temp_svcstatus->accept_passive_service_checks) ? "ENABLED" : "DISABLED");

		printf("<TR><TD CLASS='dataVar'>%s:</TD><td CLASS='dataVal'><DIV CLASS='checks%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n",_("Obsessing"), (temp_svcstatus->obsess_over_service == TRUE) ? "ENABLED" : "DISABLED", (temp_svcstatus->obsess_over_service) ? "ENABLED" : "DISABLED");

		printf("<TR><td CLASS='dataVar'>%s:</TD><td CLASS='dataVal'><DIV CLASS='notifications%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Notifications"),(temp_svcstatus->notifications_enabled) ? "ENABLED" : "DISABLED", (temp_svcstatus->notifications_enabled) ? "ENABLED" : "DISABLED");

		printf("<TR><TD CLASS='dataVar'>%s:</TD><td CLASS='dataVal'><DIV CLASS='eventhandlers%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Event Handler"),(temp_svcstatus->event_handler_enabled) ? "ENABLED" : "DISABLED", (temp_svcstatus->event_handler_enabled) ? "ENABLED" : "DISABLED");

		printf("<TR><TD CLASS='dataVar'>%s:</TD><td CLASS='dataVal'><DIV CLASS='flapdetection%s'>&nbsp;&nbsp;%s&nbsp;&nbsp;</DIV></TD></TR>\n", _("Flap Detection"),(temp_svcstatus->flap_detection_enabled == TRUE) ? "ENABLED" : "DISABLED", (temp_svcstatus->flap_detection_enabled == TRUE) ? "ENABLED" : "DISABLED");
	...
	if(nagios_process_state == STATE_OK &&  is_authorized_for_read_only(&current_authdata) == FALSE) {
		...
		if(temp_svcstatus->checks_enabled) {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Disable active checks of this service"));
			}
		else {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Enable active checks of this service"));
			}
		...
		printf("&service=%s%s'>%s</a></td></tr>\n", url_encode(service_desc), (temp_svcstatus->checks_enabled == TRUE) ? "&force_check" : "",_("Re-schedule the next check of this service"));
		...
		if(temp_svcstatus->accept_passive_service_checks == TRUE) {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Submit passive check result for this service"));
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Stop accepting passive checks for this service"));
			}
		else {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Start accepting passive checks for this service"));
			}
		...
		if(temp_svcstatus->obsess_over_service == TRUE) {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Stop obsessing over this service"));
			}
		else {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Start obsessing over this service"));
			}
		...
		if((temp_svcstatus->status == SERVICE_WARNING || temp_svcstatus->status == SERVICE_UNKNOWN || temp_svcstatus->status == SERVICE_CRITICAL) && temp_svcstatus->state_type == HARD_STATE) {
			if(temp_svcstatus->problem_has_been_acknowledged == FALSE) {
				...
				printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Acknowledge this service problem"));
				}
			else {
				...
				printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Remove problem acknowledgement"));
				}
			}
		if(temp_svcstatus->notifications_enabled == TRUE) {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Disable notifications for this service"));
			if(temp_svcstatus->status != SERVICE_OK) {
				...
				printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Delay next service notification"));
				}
			}
		else {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Enable notifications for this service"));
			}
		...
		printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Send custom service notification"));
		...
		printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Schedule downtime for this service"));
		...
		*/
		...
		if(temp_svcstatus->event_handler_enabled == TRUE) {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Disable event handler for this service"));
			}
		else {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Enable event handler for this service"));
			}
		...
		if(temp_svcstatus->flap_detection_enabled == TRUE) {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Disable flap detection for this service"));
			}
		else {
			...
			printf("&service=%s'>%s</a></td></tr>\n", url_encode(service_desc),_("Enable flap detection for this service"));
			}
		...
		printf("<DIV CLASS='infoMessage'>%s<br>\n",_("It appears as though Nagios is not running, so commands are temporarily unavailable..."));
		printf(" <a href='%s?type=%d'>%s</a>  %s</DIV>\n", EXTINFO_CGI, DISPLAY_PROCESS_INFO,_("Click"),_("view Nagios process information"));
		}
...
void show_hostgroup_info(void) {
	...
	if(is_authorized_for_hostgroup(temp_hostgroup, &current_authdata) == FALSE) {
		...
		printf("<P><DIV CLASS='errorMessage'>%s</DIV></P>\n",_("It appears as though you do not have permission to view information for this hostgroup..."));
		printf("<P><DIV CLASS='errorDescription'>%s</DIV></P>\n",_("If you believe this is an error, check the HTTP server authentication requirements for accessing this CGI<br>and check the authorization options in your CGI configuration file."));
	...
	if(temp_hostgroup == NULL) {
		printf("<P><DIV CLASS='errorMessage'>%s</DIV></P>",_("Error: Hostgroup Not Found!"));
		return;
		}
...
void show_servicegroup_info() {
	...
	if(is_authorized_for_servicegroup(temp_servicegroup, &current_authdata) == FALSE) {
		...
		printf("<P><DIV CLASS='errorMessage'>%s</DIV></P>\n",_("It appears as though you do not have permission to view information for this servicegroup..."));
		printf("<P><DIV CLASS='errorDescription'>%s</DIV></P>\n",_("If you believe this is an error, check the HTTP server authentication requirements for accessing this CGI<br>and check the authorization options in your CGI configuration file."));
	...
	if(temp_servicegroup == NULL) {
		printf("<P><DIV CLASS='errorMessage'>%s</DIV></P>",_("Error: Servicegroup Not Found!"));
		return;
		}
...
void show_all_comments(void) {
	...
	printf("<DIV CLASS='commentTitle'>%s</DIV>\n",_("Host Comments"));
	...
	printf("%s</a></div>\n",_("Add a new host comment"));

	printf("<BR />\n");
	printf("<DIV ALIGN=CENTER>\n");
	printf("<TABLE BORDER=0 CLASS='comment'>\n");
	printf("<TR CLASS='comment'><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH></TR>\n",_("Host Name"),_("Entry Time"),_("Author"),_("Comment"),_("Comment ID"),_("Persistent"),_("Type"),_("Expires"),_("Actions"));
	...
	if(total_comments == 0)
		printf("<TR CLASS='commentOdd'><TD CLASS='commentOdd' COLSPAN=9>%s</TD></TR>",_("There are no host comments"));
	...
	printf("%s</a></div>\n",_("Add a new service comment"));

	printf("<BR />\n");
	printf("<DIV ALIGN=CENTER>\n");
	printf("<TABLE BORDER=0 CLASS='comment'>\n");
	printf("<TR CLASS='comment'><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH></TR>\n",_("Host Name"),_("Service"),_("Entry Time"),_("Author"),_("Comment"),_("Comment ID"),_("Persistent"),_("Type"),_("Expires"),_("Actions"));
	...
	if(total_comments == 0)
		printf("<TR CLASS='commentOdd'><TD CLASS='commentOdd' COLSPAN=10>%s</TD></TR>",_("There are no service comments"));
...
void show_performance_data(void) {
	...
	printf("<tr class='data'><th class='data'>%s</th><th class='data'>%s</th></tr>\n",_("Time Frame"),_("Services Checked"));
	printf("<tr><td class='dataVar'>&lt;= 1 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("minute"),active_service_checks_1min, (double)(((double)active_service_checks_1min * 100.0) / (double)total_active_service_checks));
	printf("<tr><td class='dataVar'>&lt;= 5 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("minutes"), active_service_checks_5min, (double)(((double)active_service_checks_5min * 100.0) / (double)total_active_service_checks));
	printf("<tr><td class='dataVar'>&lt;= 15 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("minutes"),active_service_checks_15min, (double)(((double)active_service_checks_15min * 100.0) / (double)total_active_service_checks));
	printf("<tr><td class='dataVar'>&lt;= 1 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("hour"),active_service_checks_1hour, (double)(((double)active_service_checks_1hour * 100.0) / (double)total_active_service_checks));
	printf("<tr><td class='dataVar'>%s:&nbsp;&nbsp;</td><td class='dataVal'>%d (%.1f%%)</td>", _("Since program start"),active_service_checks_start, (double)(((double)active_service_checks_start * 100.0) / (double)total_active_service_checks));
	...
	printf("<tr class='data'><th class='data'>%s</th><th class='data'>%s.</th><th class='data'>%s.</th><th class='data'>%s</th></tr>\n",_("Metric"),_("Min"),_("Max"),_("Average"));

	printf("<tr><td class='dataVar'>%s:&nbsp;&nbsp;</td><td class='dataVal'>%.2f sec</td><td class='dataVal'>%.2f sec</td><td class='dataVal'>%.3f sec</td></tr>\n", _("Check Execution Time"),min_service_execution_time, max_service_execution_time, (double)((double)total_service_execution_time / (double)total_active_service_checks));

	printf("<tr><td class='dataVar'>%s:</td><td class='dataVal'>%.2f sec</td><td class='dataVal'>%.2f sec</td><td class='dataVal'>%.3f sec</td></tr>\n", _("Check Latency"),min_service_latency, max_service_latency, (double)((double)total_service_latency / (double)total_active_service_checks));

	printf("<tr><td class='dataVar'>%s:</td><td class='dataVal'>%.2f%%</td><td class='dataVal'>%.2f%%</td><td class='dataVal'>%.2f%%</td></tr>\n", _("Percent State Change"),min_service_percent_change_a, max_service_percent_change_a, (double)((double)total_service_percent_change_a / (double)total_active_service_checks));
	...
	printf("<tr>\n");
	printf("<td valign=middle><div class='perfTypeTitle'>%s:</div></td>\n",_("Services Passively Checked"));
	printf("<td valign=top>\n");
	...
	printf("<tr class='data'><th class='data'>%s</th><th class='data'>%s</th></tr>\n",_("Time Frame"),_("Services Checked"));
	printf("<tr><td class='dataVar'>&lt;= 1 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("minute"),passive_service_checks_1min, (double)(((double)passive_service_checks_1min * 100.0) / (double)total_passive_service_checks));
	printf("<tr><td class='dataVar'>&lt;= 5 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("minute"), passive_service_checks_5min, (double)(((double)passive_service_checks_5min * 100.0) / (double)total_passive_service_checks));
	printf("<tr><td class='dataVar'>&lt;= 15 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("minutes"),passive_service_checks_15min, (double)(((double)passive_service_checks_15min * 100.0) / (double)total_passive_service_checks));
	printf("<tr><td class='dataVar'>&lt;= 1 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("hour"), passive_service_checks_1hour, (double)(((double)passive_service_checks_1hour * 100.0) / (double)total_passive_service_checks));
	printf("<tr><td class='dataVar'>%s:&nbsp;&nbsp;</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("Since program start"),passive_service_checks_start, (double)(((double)passive_service_checks_start * 100.0) / (double)total_passive_service_checks));
	...
	printf("<tr class='data'><th class='data'>%s</th><th class='data'>%s.</th><th class='data'>%s.</th><th class='data'>%s</th></tr>\n",_("Metric"),_("Min"),_("Max"),_("Average"));
	printf("<tr><td class='dataVar'>%s:&nbsp;&nbsp;</td><td class='dataVal'>%.2f%%</td><td class='dataVal'>%.2f%%</td><td class='dataVal'>%.2f%%</td></tr>\n", _("Percent State Change"),min_service_percent_change_b, max_service_percent_change_b, (double)((double)total_service_percent_change_b / (double)total_passive_service_checks));
	...
	printf("<tr class='data'><th class='data'>%s</th><th class='data'>%s</th></tr>\n",_("Time Frame"),_("Hosts Checked"));
	printf("<tr><td class='dataVar'>&lt;= 1 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("minutes"), active_host_checks_1min, (double)(((double)active_host_checks_1min * 100.0) / (double)total_active_host_checks));
	printf("<tr><td class='dataVar'>&lt;= 5 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("minutes"),active_host_checks_5min, (double)(((double)active_host_checks_5min * 100.0) / (double)total_active_host_checks));
	printf("<tr><td class='dataVar'>&lt;= 15 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>",_("minutes"), active_host_checks_15min, (double)(((double)active_host_checks_15min * 100.0) / (double)total_active_host_checks));
	printf("<tr><td class='dataVar'>&lt;= 1 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>",_("hour"), active_host_checks_1hour, (double)(((double)active_host_checks_1hour * 100.0) / (double)total_active_host_checks));
	printf("<tr><td class='dataVar'>%s:&nbsp;&nbsp;</td><td class='dataVal'>%d (%.1f%%)</td>", _("Since program start"),active_host_checks_start, (double)(((double)active_host_checks_start * 100.0) / (double)total_active_host_checks));
	...
	printf("<tr><td class='dataVar'>%s:&nbsp;&nbsp;</td><td class='dataVal'>%.2f sec</td><td class='dataVal'>%.2f sec</td><td class='dataVal'>%.3f sec</td></tr>\n",_("Check Execution Time"), min_host_execution_time, max_host_execution_time, (double)((double)total_host_execution_time / (double)total_active_host_checks));

	printf("<tr><td class='dataVar'>%s:</td><td class='dataVal'>%.2f sec</td><td class='dataVal'>%.2f sec</td><td class='dataVal'>%.3f sec</td></tr>\n", _("Check Latency"),min_host_latency, max_host_latency, (double)((double)total_host_latency / (double)total_active_host_checks));

	printf("<tr><td class='dataVar'>%s:</td><td class='dataVal'>%.2f%%</td><td class='dataVal'>%.2f%%</td><td class='dataVal'>%.2f%%</td></tr>\n",_("Percent State Change"), min_host_percent_change_a, max_host_percent_change_a, (double)((double)total_host_percent_change_a / (double)total_active_host_checks));
	...
	printf("<tr class='data'><th class='data'>%s</th><th class='data'>%s</th></tr>\n",_("Time Frame"),_("Hosts Checked"));
	printf("<tr><td class='dataVar'>&lt;= 1 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("minute"), passive_host_checks_1min, (double)(((double)passive_host_checks_1min * 100.0) / (double)total_passive_host_checks));
	printf("<tr><td class='dataVar'>&lt;= 5 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>",_("minute"), passive_host_checks_5min, (double)(((double)passive_host_checks_5min * 100.0) / (double)total_passive_host_checks));
	printf("<tr><td class='dataVar'>&lt;= 15 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>", _("minute"),passive_host_checks_15min, (double)(((double)passive_host_checks_15min * 100.0) / (double)total_passive_host_checks));
	printf("<tr><td class='dataVar'>&lt;= 1 %s:</td><td class='dataVal'>%d (%.1f%%)</td></tr>",_("hour"), passive_host_checks_1hour, (double)(((double)passive_host_checks_1hour * 100.0) / (double)total_passive_host_checks));
	printf("<tr><td class='dataVar'>%s:&nbsp;&nbsp;</td><td class='dataVal'>%d (%.1f%%)</td></tr>",_("Since program start"), passive_host_checks_start, (double)(((double)passive_host_checks_start * 100.0) / (double)total_passive_host_checks));
	...
	printf("<tr>\n");
	printf("<td valign=center><div class='perfTypeTitle'>%s:</div></td>\n",_("Check Statistics"));
	...
	printf("<tr class='data'><th class='data'>%s</th><th class='data'>%s</th><th class='data'>%s</th><th class='data'>%s</th></tr>\n",_("Type"),_("Last 1 Min"),_("Last 5 Min"),_("Last 15 Min"));
	printf("<tr><td class='dataVar'>%s</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>", _("Active Scheduled Host Checks"),program_stats[ACTIVE_SCHEDULED_HOST_CHECK_STATS][0], program_stats[ACTIVE_SCHEDULED_HOST_CHECK_STATS][1], program_stats[ACTIVE_SCHEDULED_HOST_CHECK_STATS][2]);
	printf("<tr><td class='dataVar'>%s</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>", _("Active On-Demand Host Checks"),program_stats[ACTIVE_ONDEMAND_HOST_CHECK_STATS][0], program_stats[ACTIVE_ONDEMAND_HOST_CHECK_STATS][1], program_stats[ACTIVE_ONDEMAND_HOST_CHECK_STATS][2]);
	printf("<tr><td class='dataVar'>%s</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>", _("Parallel Host Checks"),program_stats[PARALLEL_HOST_CHECK_STATS][0], program_stats[PARALLEL_HOST_CHECK_STATS][1], program_stats[PARALLEL_HOST_CHECK_STATS][2]);
	printf("<tr><td class='dataVar'>%s</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>", _("Serial Host Checks"),program_stats[SERIAL_HOST_CHECK_STATS][0], program_stats[SERIAL_HOST_CHECK_STATS][1], program_stats[SERIAL_HOST_CHECK_STATS][2]);
	printf("<tr><td class='dataVar'>%s</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>", _("Cached Host Checks"),program_stats[ACTIVE_CACHED_HOST_CHECK_STATS][0], program_stats[ACTIVE_CACHED_HOST_CHECK_STATS][1], program_stats[ACTIVE_CACHED_HOST_CHECK_STATS][2]);
	printf("<tr><td class='dataVar'>%s</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>", _("Passive Host Checks"),program_stats[PASSIVE_HOST_CHECK_STATS][0], program_stats[PASSIVE_HOST_CHECK_STATS][1], program_stats[PASSIVE_HOST_CHECK_STATS][2]);

	printf("<tr><td class='dataVar'>%s</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>", _("Active Scheduled Service Checks"),program_stats[ACTIVE_SCHEDULED_SERVICE_CHECK_STATS][0], program_stats[ACTIVE_SCHEDULED_SERVICE_CHECK_STATS][1], program_stats[ACTIVE_SCHEDULED_SERVICE_CHECK_STATS][2]);
	printf("<tr><td class='dataVar'>%s</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>",_("Active On-Demand Service Checks"), program_stats[ACTIVE_ONDEMAND_SERVICE_CHECK_STATS][0], program_stats[ACTIVE_ONDEMAND_SERVICE_CHECK_STATS][1], program_stats[ACTIVE_ONDEMAND_SERVICE_CHECK_STATS][2]);
	printf("<tr><td class='dataVar'>%s</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>", _("Cached Service Checks"),program_stats[ACTIVE_CACHED_SERVICE_CHECK_STATS][0], program_stats[ACTIVE_CACHED_SERVICE_CHECK_STATS][1], program_stats[ACTIVE_CACHED_SERVICE_CHECK_STATS][2]);
	printf("<tr><td class='dataVar'>%s</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>", _("Passive Service Checks"),program_stats[PASSIVE_SERVICE_CHECK_STATS][0], program_stats[PASSIVE_SERVICE_CHECK_STATS][1], program_stats[PASSIVE_SERVICE_CHECK_STATS][2]);

	printf("<tr><td class='dataVar'>%s</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>", _("External Commands"), program_stats[EXTERNAL_COMMAND_STATS][0], program_stats[EXTERNAL_COMMAND_STATS][1], program_stats[EXTERNAL_COMMAND_STATS][2]);
	...
	printf("<tr class='data'><th class='data'>%s</th><th class='data'>%s</th><th class='data'>%s</th><th class='data'>%s</th></tr>\n",_("Type"),_("In Use"),_("Max Used"),_("Total Available"));
	printf("<tr><td class='dataVar'>%s&nbsp;</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td><td class='dataVal'>%d</td></tr>", _("External Commands"),buffer_stats[0][1], buffer_stats[0][2], buffer_stats[0][0]);
...
void display_comments(int type) {
	...
	printf("<DIV CLASS='commentTitle'>%s %s</DIV>\n", (type == HOST_COMMENT) ? _("Host") : _("Service"),_("Comments"));
	printf("<TABLE BORDER=0>\n");
	...
	printf("<DIV ALIGN=CENTER>\n");
	printf("<TABLE BORDER=0 CLASS='comment'>\n");
	printf("<TR CLASS='comment'><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH><TH CLASS='comment'>%s</TH></TR>\n",_("Entry Time"),_("Author"),_("Comment"),_("Comment ID"),_("Persistent"),_("Type"),_("Expires"),_("Actions"));
...
void show_all_downtime(void) {
	...
	printf("<BR />\n");
	printf("<DIV CLASS='downtimeNav'>[&nbsp;<A HREF='#HOSTDOWNTIME' CLASS='downtimeNav'>%s</A>&nbsp;|&nbsp;<A HREF='#SERVICEDOWNTIME' CLASS='downtimeNav'>%s</A>&nbsp;]</DIV>\n",_("Host Downtime"),_("Service Downtime"));
	printf("<BR />\n");

	printf("<A NAME=HOSTDOWNTIME></A>\n");
	printf("<DIV CLASS='downtimeTitle'>%s</DIV>\n",_("Scheduled Host Downtime"));
	...
	printf("%s</a></div>\n",_("Schedule host downtime"));

	printf("<BR />\n");
	printf("<DIV ALIGN=CENTER>\n");
	printf("<TABLE BORDER=0 CLASS='downtime'>\n");
	printf("<TR CLASS='downtime'><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH></TR>\n",_("Host Name"),_("Entry Time"),_("Author"),_("Comment"),_("Start Time"),_("End Time"),_("Type"),_("Duration"),_("Downtime ID"),_("Trigger ID"),_("Actions"));
	...
	if(total_downtime == 0)
		printf("<TR CLASS='downtimeOdd'><TD CLASS='downtimeOdd' COLSPAN=11>%s</TD></TR>",_("There are no hosts with scheduled downtime"));
	...
	printf("<DIV CLASS='downtimeTitle'>%s</DIV>\n",_("Scheduled Service Downtime"));

	printf("<div CLASS='comment'><img src='%s%s' border=0>&nbsp;", url_images_path, DOWNTIME_ICON);
	printf("<a href='%s?cmd_typ=%d'>", COMMAND_CGI, CMD_SCHEDULE_SVC_DOWNTIME);
	printf("%s</a></div>\n",_("Schedule service downtime"));

	printf("<BR />\n");
	printf("<DIV ALIGN=CENTER>\n");
	printf("<TABLE BORDER=0 CLASS='downtime'>\n");
	printf("<TR CLASS='downtime'><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH><TH CLASS='downtime'>%s</TH></TR>\n",_("Host Name"),_("Service"),_("Entry Time"),_("Author"),_("Comment"),_("Start Time"),_("End Time"),_("Type"),_("Duration"),_("Downtime ID"),_("Trigger ID"),_("Actions"));
	...
	if(total_downtime == 0)
		printf("<TR CLASS='downtimeOdd'><TD CLASS='downtimeOdd' COLSPAN=12>%s</TD></TR>",_("There are no services with scheduled downtime"));
...
void show_scheduling_queue(void) {
	...
	printf("<DIV ALIGN=CENTER CLASS='statusSort'>%s <b>",_("Entries sorted by"));
	if(sort_option == SORT_HOSTNAME)
		printf("%s",_("host name"));
	else if(sort_option == SORT_SERVICENAME)
		printf("%s",_("service name"));
	else if(sort_option == SORT_SERVICESTATUS)
		printf("%s",_("service status"));
	else if(sort_option == SORT_LASTCHECKTIME)
		printf("%s",_("last check time"));
	else if(sort_option == SORT_NEXTCHECKTIME)
		printf("%s",_("next check time"));
	...
	printf("<TH CLASS='queue'>%s&nbsp;<A HREF='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' BORDER=0 ALT='Sort by host name (ascending)' TITLE='Sort by host name (ascending)'></A><A HREF='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' BORDER=0 ALT='Sort by host name (descending)' TITLE='Sort by host name (descending)'></A></TH>", _("Host"),temp_url, SORT_ASCENDING, SORT_HOSTNAME, url_images_path, UP_ARROW_ICON, temp_url, SORT_DESCENDING, SORT_HOSTNAME, url_images_path, DOWN_ARROW_ICON);

	printf("<TH CLASS='queue'>%s&nbsp;<A HREF='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' BORDER=0 ALT='Sort by service name (ascending)' TITLE='Sort by service name (ascending)'></A><A HREF='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' BORDER=0 ALT='Sort by service name (descending)' TITLE='Sort by service name (descending)'></A></TH>", _("Service"),temp_url, SORT_ASCENDING, SORT_SERVICENAME, url_images_path, UP_ARROW_ICON, temp_url, SORT_DESCENDING, SORT_SERVICENAME, url_images_path, DOWN_ARROW_ICON);

	printf("<TH CLASS='queue'>%s&nbsp;<A HREF='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' BORDER=0 ALT='Sort by last check time (ascending)' TITLE='Sort by last check time (ascending)'></A><A HREF='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' BORDER=0 ALT='Sort by last check time (descending)' TITLE='Sort by last check time (descending)'></A></TH>", _("Last Check"),temp_url, SORT_ASCENDING, SORT_LASTCHECKTIME, url_images_path, UP_ARROW_ICON, temp_url, SORT_DESCENDING, SORT_LASTCHECKTIME, url_images_path, DOWN_ARROW_ICON);

	printf("<TH CLASS='queue'>%s&nbsp;<A HREF='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' BORDER=0 ALT='Sort by next check time (ascending)' TITLE='Sort by next check time (ascending)'></A><A HREF='%s&sorttype=%d&sortoption=%d'><IMG SRC='%s%s' BORDER=0 ALT='Sort by next check time (descending)' TITLE='Sort by next check time (descending)'></A></TH>",_("Next Check"), temp_url, SORT_ASCENDING, SORT_NEXTCHECKTIME, url_images_path, UP_ARROW_ICON, temp_url, SORT_DESCENDING, SORT_NEXTCHECKTIME, url_images_path, DOWN_ARROW_ICON);


	printf("<TH CLASS='queue'>%s</TH><TH CLASS='queue'>%s</TH><TH CLASS='queue'>%s</TH></TR>\n",_("Type"),_("Active Checks"),_("Actions"));
