...
int main(int argc, char **argv) {
	...
	if(display_header == TRUE) {
		...
		switch(display_type) {
			case DISPLAY_HOST_AVAIL:
				snprintf(temp_buffer, sizeof(temp_buffer) - 1, _("Host Availability Report"));
				break;
			case DISPLAY_SERVICE_AVAIL:
				snprintf(temp_buffer, sizeof(temp_buffer) - 1, _("Service Availability Report"));
				break;
			case DISPLAY_HOSTGROUP_AVAIL:
				snprintf(temp_buffer, sizeof(temp_buffer) - 1, _("Hostgroup Availability Report"));
				break;
			case DISPLAY_SERVICEGROUP_AVAIL:
				snprintf(temp_buffer, sizeof(temp_buffer) - 1, _("Servicegroup Availability Report"));
				break;
			default:
				snprintf(temp_buffer, sizeof(temp_buffer) - 1, _("Availability Report"));
				break;
			}
		...
		if(((display_type == DISPLAY_HOST_AVAIL && show_all_hosts == FALSE) || (display_type == DISPLAY_SERVICE_AVAIL && show_all_services == FALSE)) && get_date_parts == FALSE) {
			...
			if(display_type == DISPLAY_HOST_AVAIL && show_all_hosts == FALSE) {
				...
				printf("<BR>\n");
#ifdef USE_TRENDS
				printf("<a href='%s?host=%s&t1=%lu&t2=%lu&assumestateretention=%s&assumeinitialstates=%s&includesoftstates=%s&assumestatesduringnotrunning=%s&initialassumedhoststate=%d&backtrack=%d'>%s</a><BR>\n", TRENDS_CGI, url_encode(host_name), t1, t2, (include_soft_states == TRUE) ? "yes" : "no", (assume_state_retention == TRUE) ? "yes" : "no", (assume_initial_states == TRUE) ? "yes" : "no", (assume_states_during_notrunning == TRUE) ? "yes" : "no", initial_assumed_host_state, backtrack_archives,_("View Trends For This Host"));
#endif
#ifdef USE_HISTOGRAM
				printf("<a href='%s?host=%s&t1=%lu&t2=%lu&assumestateretention=%s'>%s</a><BR>\n", HISTOGRAM_CGI, url_encode(host_name), t1, t2, (assume_state_retention == TRUE) ? "yes" : "no",_("View Alert Histogram For This Host"));
#endif
				printf("<a href='%s?host=%s'>%s</a><BR>\n", STATUS_CGI, url_encode(host_name),_("View Alert Histogram For This Host"));
				...
				printf("&service=%s&t1=%lu&t2=%lu&assumestateretention=%s&includesoftstates=%s&assumeinitialstates=%s&assumestatesduringnotrunning=%s&initialassumedservicestate=%d&backtrack=%d'>%s</a><BR>\n", url_encode(svc_description), t1, t2, (include_soft_states == TRUE) ? "yes" : "no", (assume_state_retention == TRUE) ? "yes" : "no", (assume_initial_states == TRUE) ? "yes" : "no", (assume_states_during_notrunning == TRUE) ? "yes" : "no", initial_assumed_service_state, backtrack_archives,_("View Trends For This Service"));
#endif
#ifdef USE_HISTOGRAM
				printf("<a href='%s?host=%s", HISTOGRAM_CGI, url_encode(host_name));
				printf("&service=%s&t1=%lu&t2=%lu&assumestateretention=%s'>%s</a><BR>\n", url_encode(svc_description), t1, t2, (assume_state_retention == TRUE) ? "yes" : "no",_("View Alert Histogram For This Service"));
#endif
				printf("<A HREF='%s?host=%s&", HISTORY_CGI, url_encode(host_name));
				printf("service=%s'>%s</A><BR>\n", url_encode(svc_description),_("View Alert History For This Service"));
				printf("<A HREF='%s?host=%s&", NOTIFICATIONS_CGI, url_encode(host_name));
				printf("service=%s'>%s</A><BR>\n", url_encode(svc_description),_("View Notifications For This Service"));
				}
			printf("<a href='javascript:window.print()' target='_self'>print current page</a>");
			printf("</TD></TR>\n");
			printf("</TABLE>\n");
			}
		...
		if(display_type != DISPLAY_NO_AVAIL && get_date_parts == FALSE) {
			...
			if(display_type == DISPLAY_HOST_AVAIL) {
				if(show_all_hosts == TRUE)
					printf("%s",_("All Hosts"));
				else
					printf("%s '%s'",_("Host"),host_name);
				}
			else if(display_type == DISPLAY_SERVICE_AVAIL) {
				if(show_all_services == TRUE)
					printf("%s",_("All Services"));
				else
					printf("%s '%s' %s '%s'", _("Service"),svc_description,_("On Host"), host_name);
				}
			else if(display_type == DISPLAY_HOSTGROUP_AVAIL) {
				if(show_all_hostgroups == TRUE)
					printf("%s",_("All Hostgroups"));
				else
					printf("%s '%s'",_("Hostgroup"), hostgroup_name);
				}
			else if(display_type == DISPLAY_SERVICEGROUP_AVAIL) {
				if(show_all_servicegroups == TRUE)
					printf("%s",_("All Servicegroups"));
				else
					printf("%s '%s'", _("Servicegroup"),servicegroup_name);
				}
			printf("</DIV>\n");
			...
			printf("<div align=center class='reportRange'>%s %s %s</div>\n", start_timestring,_("to"), end_timestring);

			get_time_breakdown((time_t)(t2 - t1), &days, &hours, &minutes, &seconds);
			printf("<div align=center class='reportDuration'>%s: %dd %dh %dm %ds</div>\n",_("Duration"), days, hours, minutes, seconds);
			}
		...
		if(display_type != DISPLAY_NO_AVAIL && get_date_parts == FALSE) {
			...
			if(display_type == DISPLAY_HOST_AVAIL || display_type == DISPLAY_HOSTGROUP_AVAIL || display_type == DISPLAY_SERVICEGROUP_AVAIL) {
				...
				printf("<option value=%d %s>%s\n", AS_NO_DATA, (initial_assumed_host_state == AS_NO_DATA) ? "SELECTED" : "",_("Unspecified"));
				printf("<option value=%d %s>%s\n", AS_CURRENT_STATE, (initial_assumed_host_state == AS_CURRENT_STATE) ? "SELECTED" : "",_("Current State"));
				printf("<option value=%d %s>%s\n", AS_HOST_UP, (initial_assumed_host_state == AS_HOST_UP) ? "SELECTED" : "",_("Host Up"));
				printf("<option value=%d %s>%s\n", AS_HOST_DOWN, (initial_assumed_host_state == AS_HOST_DOWN) ? "SELECTED" : "",_("Host Down"));
				printf("<option value=%d %s>%s\n", AS_HOST_UNREACHABLE, (initial_assumed_host_state == AS_HOST_UNREACHABLE) ? "SELECTED" : "",_("Host Unreachable"));
				printf("</select>\n");
				}
			else {
				...
				printf("<option value=%d %s>%s\n", AS_NO_DATA, (initial_assumed_service_state == AS_NO_DATA) ? "SELECTED" : "",_("Unspecified"));
				printf("<option value=%d %s>%s\n", AS_CURRENT_STATE, (initial_assumed_service_state == AS_CURRENT_STATE) ? "SELECTED" : "",_("Current State"));
				printf("<option value=%d %s>%s\n", AS_SVC_OK, (initial_assumed_service_state == AS_SVC_OK) ? "SELECTED" : "",_("Service Ok"));
				printf("<option value=%d %s>%s\n", AS_SVC_WARNING, (initial_assumed_service_state == AS_SVC_WARNING) ? "SELECTED" : "",_("Service Warning"));
				printf("<option value=%d %s>%s\n", AS_SVC_UNKNOWN, (initial_assumed_service_state == AS_SVC_UNKNOWN) ? "SELECTED" : "",_("Service Unknown"));
				printf("<option value=%d %s>%s\n", AS_SVC_CRITICAL, (initial_assumed_service_state == AS_SVC_CRITICAL) ? "SELECTED" : "",_("Service Critical"));
				printf("</select>\n");
				}
			printf("</td>\n");
	...
	if(get_date_parts == TRUE) {
		...
		printf("<P><DIV ALIGN=CENTER CLASS='dateSelectTitle'>%s</DIV></p>\n",_("Step 3: Select Report Options"));
		...
		printf("<tr>");
		printf("<td valign=top class='reportSelectSubTitle'>%s:</td>\n",_("Report Period"));
		printf("<td valign=top align=left class='optBoxItem'>\n");
		printf("<select name='timeperiod'>\n");
		printf("<option value=today>%s\n",_("Today"));
		printf("<option value=last24hours>%s\n",_("Last 24 Hours"));
		printf("<option value=yesterday>%s\n",_("Yesterday"));
		printf("<option value=thisweek>%s\n",_("This Week"));
		printf("<option value=last7days SELECTED>%s\n",_("Last 7 Days"));
		printf("<option value=lastweek>%s\n",_("Last 7 Days"));
		printf("<option value=thismonth>%s\n",_("This Month"));
		printf("<option value=last31days>%s\n",_("Last 31 Days"));
		printf("<option value=lastmonth>%s\n",_("Last Month"));
		printf("<option value=thisyear>%s\n",_("This Year"));
		printf("<option value=lastyear>%s\n",_("This Year"));
		printf("<option value=custom>* %s *\n",_("CUSTOM REPORT PERIOD"));
		printf("</select>\n");
		printf("</td>\n");
		printf("</tr>\n");

		printf("<tr><td valign=top class='reportSelectSubTitle'>%s...</td></tr>\n",_("If Custom Report Period"));

		printf("<tr>");
		printf("<td valign=top class='reportSelectSubTitle'>%s:</td>\n",_("Start Date (Inclusive)"));
		printf("<td align=left valign=top class='reportSelectItem'>");
		printf("<select name='smon'>\n");
		printf("<option value='1' %s>%s\n", (t->tm_mon == 0) ? "SELECTED" : "",_("January"));
		printf("<option value='2' %s>%s\n", (t->tm_mon == 1) ? "SELECTED" : "",_("February"));
		printf("<option value='3' %s>%s\n", (t->tm_mon == 2) ? "SELECTED" : "",_("March"));
		printf("<option value='4' %s>%s\n", (t->tm_mon == 3) ? "SELECTED" : "",_("April"));
		printf("<option value='5' %s>%s\n", (t->tm_mon == 4) ? "SELECTED" : "",_("May"));
		printf("<option value='6' %s>%s\n", (t->tm_mon == 5) ? "SELECTED" : "",_("June"));
		printf("<option value='7' %s>%s\n", (t->tm_mon == 6) ? "SELECTED" : "",_("July"));
		printf("<option value='8' %s>%s\n", (t->tm_mon == 7) ? "SELECTED" : "",_("August"));
		printf("<option value='9' %s>%s\n", (t->tm_mon == 8) ? "SELECTED" : "",_("September"));
		printf("<option value='10' %s>%s\n", (t->tm_mon == 9) ? "SELECTED" : "",_("October"));
		printf("<option value='11' %s>%s\n", (t->tm_mon == 10) ? "SELECTED" : "",_("November"));
		printf("<option value='12' %s>%s\n", (t->tm_mon == 11) ? "SELECTED" : "",_("December"));
		printf("</select>\n ");
		...
		printf("<tr><td class='reportSelectSubTitle' align=right>%s:</td>\n",_("Assume Initial States:"));
		printf("<td class='reportSelectItem'>\n");
		printf("<select name='assumeinitialstates'>\n");
		printf("<option value=yes>%s\n",_("Yes"));
		printf("<option value=no>%s\n",_("No"));
		printf("</select>\n");
		printf("</td></tr>\n");
		...
		printf("<option value=yes>%s\n",_("Yes"));
		printf("<option value=no SELECTED>%s\n",_("No"));
		printf("</select>\n");
		printf("</td></tr>\n");
		...
		if(display_type != DISPLAY_SERVICE_AVAIL) {
			...
			printf("<td class='reportSelectItem'>\n");
			printf("<select name='initialassumedhoststate'>\n");
			printf("<option value=%d>%s\n", AS_NO_DATA,_("Unspecified"));
			printf("<option value=%d>%s\n", AS_CURRENT_STATE,_("Current State"));
			printf("<option value=%d>%s\n", AS_HOST_UP,_("Host Up"));
			printf("<option value=%d>%s\n", AS_HOST_DOWN,_("Host Down"));
			printf("<option value=%d>%s\n", AS_HOST_UNREACHABLE,_("Host Unreachable"));
			printf("</select>\n");
			printf("</td></tr>\n");
			}
		...
		printf("<option value=%d>%s\n", AS_NO_DATA,_("Unspecified"));
		printf("<option value=%d>%s\n", AS_CURRENT_STATE,_("Current State"));
		printf("<option value=%d>%s\n", AS_SVC_OK,_("Service Ok"));
		printf("<option value=%d>%s\n", AS_SVC_WARNING,_("Service Warning"));
		printf("<option value=%d>%s\n", AS_SVC_UNKNOWN,_("Service Unknown"));
		printf("<option value=%d>%s\n", AS_SVC_CRITICAL,_("Service Critical"));
		printf("</select>\n");
		printf("</td></tr>\n");

		printf("<tr><td class='reportSelectSubTitle' align=right>%s:</td>\n",_("Backtracked Archives (To Scan For Initial States)"));
		printf("<td class='reportSelectItem'>\n");
		...
		printf("<tr><td></td><td align=left class='dateSelectItem'><input type='submit' value='%s'></td></tr>\n",_("Create Availability Report!"));
	...
	/* step 2 - the user wants to select a hostgroup */
		...
		printf("<p><div align=center class='reportSelectTitle'>%s</div></p>\n",_("Step 2: Select Hostgroup"));
		...
		printf("<tr><td class='reportSelectSubTitle' valign=center>%s:</td><td align=left valign=center class='reportSelectItem'>\n",_("Hostgroup(s)"));
	...
	/* step 2 - the user wants to select a service */
		...
		printf("<div align=center class='helpfulHint'>%s\n",_("Tip: If you want to have the option of getting the availability data in CSV format, select '<b>** ALL SERVICES **</b>' from the pull-down menu."));
		}
	...
	/* generate availability report */
		...
		if(is_authorized == FALSE)
			printf("<P><DIV ALIGN=CENTER CLASS='errorMessage'>%s %s...</DIV></P>\n", _("It appears as though you are not authorized to view information for the specified"),(display_type == DISPLAY_HOST_AVAIL) ? "host" : "service");
			...
			if(output_format == HTML_OUTPUT) {
				...
				printf("<div align=center class='reportTime'>[ %s %d  %d %s ]</div>\n", _("Availability report completed in"),minutes,_("min"), seconds,_("sec"));
				printf("<BR><BR>\n");
				}
	...
	/* step 1 - ask the user what kind of report they want */
		...
		printf("<option value=hostgroups>%s\n",_("Hostgroup(s)"));
		printf("<option value=hosts>%s\n",_("Host(s)"));
		printf("<option value=servicegroups>%s\n",_("Servicegroup(s)"));
		printf("<option value=services>%s\n",_("Service(s)"));
		printf("</select>\n");
		printf("</td></tr>\n");
...
#ifdef DEBUG
			printf("%s=%d<br>\n",_("SETTING LAST KNOWN STATE"), subject->last_known_state);
#endif
			}
...
#ifdef DEBUG
			printf("%s: %d @ %lu<br>\n",_("SKIPPING PRE-EVENT"), temp_as->entry_type, temp_as->time_stamp);
#endif
			last_as = temp_as;
			continue;
			}
...
#ifdef DEBUG
			printf("<b>%s: %ld -> %ld. (%ld -> %ld)</b><br>\n", _("Matching in timerange"),temp_start, temp_end, midnight_today + temp_start, midnight_today + temp_end);
#endif
...
#ifdef DEBUG
				printf("<li>%s[%d]: %d -> %d (%ld -> %ld)<br>\n",_("Matching in timerange"), weekday, temp_timerange->range_start, temp_timerange->range_end, temp_start, temp_end);
#endif
...
#ifdef DEBUG
					printf("<li>%s: %ld -> %ld = %d<br>\n", _("Matched time"),start, end, temp_duration);
#endif
					}
#ifdef DEBUG
				else
					printf("<li>%s: %ld -> %ld<br>\n", _("Ignored time"),start, end);
#endif
				}
...
#ifdef DEBUG
	printf("%s: %s=%lu, %s=%lu\n",_("PASSED TIME CHECKS, CLIPPED VALUES"), _("START"),start_time, _("END"),end_time);
#endif
...
#ifdef DEBUG2
	printf("\t%s: %s '%s', %s '%s', %s=%d, %s=%lu, %s=%lu\n", _("SUBJECT DOWNTIME"),_("Host"),subject->host_name,_("Service"), (subject->service_description == NULL) ? "NULL" : subject->service_description,_("State"), subject_state, _("Duration"),state_duration, _("Start"),start_time);
#endif
...
#ifdef DEBUG
		printf("%s #%d\n",_("Reading archive"), current_archive);
#endif
...
void write_log_entries(avail_subject *subject) {
	...
	printf("<DIV ALIGN=CENTER CLASS='dataTitle'>%s %s:</DIV>\n", (subject->type == HOST_SUBJECT) ? "Host" : "Service",_("Log Entries"));
	...
	printf("<tr><th class='logEntries'>%s</th><th class='logEntries'>%s</th><th class='logEntries'>%s</th><th class='logEntries'>%s</th><th class='logEntries'>%s</th></tr>\n",_("Event Start Time"),_("Event End Time"),_("Event Duration"),_("Event/State Type"),_("Event/State Information"));
...
void display_specific_hostgroup_availability(hostgroup *hg) {
	...
	printf("<BR><BR>\n");
	printf("<DIV ALIGN=CENTER CLASS='dataTitle'>%s '%s' %s:</DIV>\n",_("Hostgroup"), hg->group_name,_("Host State Breakdowns"));

	printf("<DIV ALIGN=CENTER>\n");
	printf("<TABLE BORDER=0 CLASS='data'>\n");
	printf("<TR><TH CLASS='data'>%s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH></TR>\n",_("Host"),_("Time Up"),_("Time Down"),_("Time Unreachable"),_("Time Undetermined"));
	...
	printf("<tr CLASS='data%s'><td CLASS='data%s'>%s</td><td CLASS='hostUP'>%2.3f%% (%2.3f%%)</td><td CLASS='hostDOWN'>%2.3f%% (%2.3f%%)</td><td CLASS='hostUNREACHABLE'>%2.3f%% (%2.3f%%)</td><td class='data%s'>%2.3f%%</td></tr>", bgclass, bgclass, _("Average"),average_percent_time_up, average_percent_time_up_known, average_percent_time_down, average_percent_time_down_known, average_percent_time_unreachable, average_percent_time_unreachable_known, bgclass, average_percent_time_indeterminate);
...
void display_specific_servicegroup_availability(servicegroup *sg) {
	...
	printf("<BR><BR>\n");
	printf("<DIV ALIGN=CENTER CLASS='dataTitle'>%s '%s' %s:</DIV>\n", _("Servicegroup"),sg->group_name,_("Host State Breakdowns"));
	...
	printf("<BR>\n");
	printf("<DIV ALIGN=CENTER CLASS='dataTitle'>%s '%s' %s:</DIV>\n", _("Servicegroup"),sg->group_name,_("Service State Breakdowns"));

	printf("<DIV ALIGN=CENTER>\n");
	printf("<TABLE BORDER=0 CLASS='data'>\n");
	printf("<TR><TH CLASS='data'>%s</TH><TH CLASS='data'>%s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH></TR>\n",_("Host"),_("Service"),_("Time OK"),_("Time Warning"),_("Time Unknown"),_("Time Critical"),_("Time Undetermined"));
	...
	printf("<tr CLASS='data%s'><td CLASS='data%s' colspan='2'>%s</td><td CLASS='serviceOK'>%2.3f%% (%2.3f%%)</td><td CLASS='serviceWARNING'>%2.3f%% (%2.3f%%)</td><td CLASS='serviceUNKNOWN'>%2.3f%% (%2.3f%%)</td><td class='serviceCRITICAL'>%2.3f%% (%2.3f%%)</td><td class='data%s'>%2.3f%%</td></tr>\n", bgclass, bgclass,_("Average"), average_percent_time_ok, average_percent_time_ok_known, average_percent_time_warning, average_percent_time_warning_known, average_percent_time_unknown, average_percent_time_unknown_known, average_percent_time_critical, average_percent_time_critical_known, bgclass, average_percent_time_indeterminate);
...
void display_host_availability(void) {
...
#ifdef DEBUG
	printf("%s: '%ld' %s<br>\n", _("Total time"),total_time,_("seconds"));
#endif
	...
	if(show_all_hosts == FALSE) {
		...
		printf("<DIV ALIGN=CENTER CLASS='dataTitle'>%s:</DIV>\n",_("Host State Breakdowns"));
		...
		printf("<DIV ALIGN=CENTER>\n");
		printf("<TABLE BORDER=0 CLASS='data'>\n");
		printf("<TR><TH CLASS='data'>%s</TH><TH CLASS='data'>%s</TH><TH CLASS='data'>%s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH></TR>\n",_("State"),_("Type / Reason"),_("Time"),_("Total Time"),_("Total Time"));

		/* up times */
		printf("<tr CLASS='dataEven'><td CLASS='hostUP' rowspan=3>UP</td>");
		printf("<td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td class='dataEven'>%2.3f%%</td></tr>\n", _("Unscheduled"), time_up_unscheduled_string, percent_time_up, percent_time_up_known);
		printf("<tr CLASS='dataEven'><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td class='dataEven'>%2.3f%%</td></tr>\n", _("Scheduled"), time_up_scheduled_string, percent_time_up_scheduled, percent_time_up_scheduled_known);
		printf("<tr CLASS='hostUP'><td CLASS='hostUP'>%s</td><td CLASS='hostUP'>%s</td><td CLASS='hostUP'>%2.3f%%</td><td class='hostUP'>%2.3f%%</td></tr>\n", _("Total"), time_up_string, percent_time_up, percent_time_up_known);

		/* down times */
		printf("<tr CLASS='dataOdd'><td CLASS='hostDOWN' rowspan=3>%s</td>",_("DOWN"));
		printf("<td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%2.3f%%</td><td class='dataOdd'>%2.3f%%</td></tr>\n", _("Unscheduled"),time_down_unscheduled_string, percent_time_down_unscheduled, percent_time_down_unscheduled_known);
		printf("<tr CLASS='dataOdd'><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%2.3f%%</td><td class='dataOdd'>%2.3f%%</td></tr>\n", _("Scheduled"), time_down_scheduled_string, percent_time_down_scheduled, percent_time_down_scheduled_known);
		printf("<tr CLASS='hostDOWN'><td CLASS='hostDOWN'>%s</td><td CLASS='hostDOWN'>%s</td><td CLASS='hostDOWN'>%2.3f%%</td><td class='hostDOWN'>%2.3f%%</td></tr>\n", _("Total"), time_down_string, percent_time_down, percent_time_down_known);

		/* unreachable times */
		printf("<tr CLASS='dataEven'><td CLASS='hostUNREACHABLE' rowspan=3>%s</td>",_("UNREACHABLE"));
		printf("<td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td class='dataEven'>%2.3f%%</td></tr>\n", _("Unscheduled"), time_unreachable_unscheduled_string, percent_time_unreachable, percent_time_unreachable_known);
		printf("<tr CLASS='dataEven'><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td class='dataEven'>%2.3f%%</td></tr>\n", _("Scheduled"),time_unreachable_scheduled_string, percent_time_unreachable_scheduled, percent_time_unreachable_scheduled_known);
		printf("<tr CLASS='hostUNREACHABLE'><td CLASS='hostUNREACHABLE'>%s</td><td CLASS='hostUNREACHABLE'>%s</td><td CLASS='hostUNREACHABLE'>%2.3f%%</td><td class='hostUNREACHABLE'>%2.3f%%</td></tr>\n", _("Total"), time_unreachable_string, percent_time_unreachable, percent_time_unreachable_known);

		/* indeterminate times */
		printf("<tr CLASS='dataOdd'><td CLASS='dataOdd' rowspan=3>%s</td>",_("Undetermined"));
		printf("<td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%2.3f%%</td><td CLASS='dataOdd'></td></tr>\n", _("Nagios Not Running"),time_indeterminate_notrunning_string, percent_time_indeterminate_notrunning);
		printf("<tr CLASS='dataOdd'><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%2.3f%%</td><td CLASS='dataOdd'></td></tr>\n", _("Insufficient Data"),time_indeterminate_nodata_string, percent_time_indeterminate_nodata);
		printf("<tr CLASS='dataOdd'><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%2.3f%%</td><td CLASS='dataOdd'></td></tr>\n", _("Total"), time_indeterminate_string, percent_time_indeterminate);

		printf("<tr><td colspan=3></td></tr>\n");

		printf("<tr CLASS='dataEven'><td CLASS='dataEven'>%s</td><td class='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>100.000%%</td><td CLASS='dataEven'>100.000%%</td></tr>\n",_("All"),_("Total"), total_time_string);
		printf("</table>\n");
		printf("</DIV>\n");
		...
		printf("<BR><BR>\n");
		printf("<DIV ALIGN=CENTER CLASS='dataTitle'>%s:</DIV>\n",_("State Breakdowns For Host Services"));

		printf("<DIV ALIGN=CENTER>\n");
		printf("<TABLE BORDER=0 CLASS='data'>\n");
		printf("<TR><TH CLASS='data'>%s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH></TR>\n",_("Service"),_("Time OK"),_("Time Warning"),_("Time Unknown"),_("Time Critical"),_("Time Undetermined"));
		...
		printf("<tr CLASS='data%s'><td CLASS='data%s'>%s</td><td CLASS='serviceOK'>%2.3f%% (%2.3f%%)</td><td CLASS='serviceWARNING'>%2.3f%% (%2.3f%%)</td><td CLASS='serviceUNKNOWN'>%2.3f%% (%2.3f%%)</td><td class='serviceCRITICAL'>%2.3f%% (%2.3f%%)</td><td class='data%s'>%2.3f%%</td></tr>\n", bgclass, bgclass,_("Average"), average_percent_time_ok, average_percent_time_ok_known, average_percent_time_warning, average_percent_time_warning_known, average_percent_time_unknown, average_percent_time_unknown_known, average_percent_time_critical, average_percent_time_critical_known, bgclass, average_percent_time_indeterminate);
...
void display_service_availability(void) {
	...
	if(show_all_services == FALSE) {
		...
		printf("<DIV ALIGN=CENTER>\n");
		printf("<TABLE BORDER=0 CLASS='data'>\n");
		printf("<TR><TH CLASS='data'>%s</TH><TH CLASS='data'>%s</TH><TH CLASS='data'>%s</TH><TH CLASS='data'>%% %s</TH><TH CLASS='data'>%% %s</TH></TR>\n",_("State"),_("Type / Reason"),_("Time"),_("Total Time"),_("Known Time"));

		/* ok states */
		printf("<tr CLASS='dataEven'><td CLASS='serviceOK' rowspan=3>OK</td>");
		printf("<td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td CLASS='dataEven'>%2.3f%%</td></tr>\n",_("Unscheduled"), time_ok_unscheduled_string, percent_time_ok_unscheduled, percent_time_ok_unscheduled_known);
		printf("<tr CLASS='dataEven'><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td CLASS='dataEven'>%2.3f%%</td></tr>\n",_("Scheduled"), time_ok_scheduled_string, percent_time_ok_scheduled, percent_time_ok_scheduled_known);
		printf("<tr CLASS='serviceOK'><td CLASS='serviceOK'>%s</td><td CLASS='serviceOK'>%s</td><td CLASS='serviceOK'>%2.3f%%</td><td CLASS='serviceOK'>%2.3f%%</td></tr>\n",_("Total"), time_ok_string, percent_time_ok, percent_time_ok_known);

		/* warning states */
		printf("<tr CLASS='dataOdd'><td CLASS='serviceWARNING' rowspan=3>WARNING</td>");
		printf("<td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%2.3f%%</td><td CLASS='dataOdd'>%2.3f%%</td></tr>\n",_("Unscheduled"), time_warning_unscheduled_string, percent_time_warning_unscheduled, percent_time_warning_unscheduled_known);
		printf("<tr CLASS='dataOdd'><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%2.3f%%</td><td CLASS='dataOdd'>%2.3f%%</td></tr>\n", _("Scheduled"),time_warning_scheduled_string, percent_time_warning_scheduled, percent_time_warning_scheduled_known);
		printf("<tr CLASS='serviceWARNING'><td CLASS='serviceWARNING'>%s</td><td CLASS='serviceWARNING'>%s</td><td CLASS='serviceWARNING'>%2.3f%%</td><td CLASS='serviceWARNING'>%2.3f%%</td></tr>\n", _("Total"), time_warning_string, percent_time_warning, percent_time_warning_known);

		/* unknown states */
		printf("<tr CLASS='dataEven'><td CLASS='serviceUNKNOWN' rowspan=3>UNKNOWN</td>");
		printf("<td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td CLASS='dataEven'>%2.3f%%</td></tr>\n", _("Unscheduled"),time_unknown_unscheduled_string, percent_time_unknown_unscheduled, percent_time_unknown_unscheduled_known);
		printf("<tr CLASS='dataEven'><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td CLASS='dataEven'>%2.3f%%</td></tr>\n", _("Scheduled"),time_unknown_scheduled_string, percent_time_unknown_scheduled, percent_time_unknown_scheduled_known);
		printf("<tr CLASS='serviceUNKNOWN'><td CLASS='serviceUNKNOWN'>%s</td><td CLASS='serviceUNKNOWN'>%s</td><td CLASS='serviceUNKNOWN'>%2.3f%%</td><td CLASS='serviceUNKNOWN'>%2.3f%%</td></tr>\n", _("Total"), time_unknown_string, percent_time_unknown, percent_time_unknown_known);

		/* critical states */
		printf("<tr CLASS='dataOdd'><td CLASS='serviceCRITICAL' rowspan=3>CRITICAL</td>");
		printf("<td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%2.3f%%</td><td CLASS='dataOdd'>%2.3f%%</td></tr>\n", _("Unscheduled"),time_critical_unscheduled_string, percent_time_critical_unscheduled, percent_time_critical_unscheduled_known);
		printf("<tr CLASS='dataOdd'><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%2.3f%%</td><td CLASS='dataOdd'>%2.3f%%</td></tr>\n", _("Scheduled"),time_critical_scheduled_string, percent_time_critical_scheduled, percent_time_critical_scheduled_known);
		printf("<tr CLASS='serviceCRITICAL'><td CLASS='serviceCRITICAL'>%s</td><td CLASS='serviceCRITICAL'>%s</td><td CLASS='serviceCRITICAL'>%2.3f%%</td><td CLASS='serviceCRITICAL'>%2.3f%%</td></tr>\n", _("Total"),time_critical_string, percent_time_critical, percent_time_critical_known);


		printf("<tr CLASS='dataEven'><td CLASS='dataEven' rowspan=3>%s</td>",_("Undetermined"));
		/*
		printf("<td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td CLASS='dataEven'></td></tr>\n",_("Unscheduled"),time_indeterminate_unscheduled_string,percent_time_indeterminate_unscheduled);
		printf("<tr CLASS='dataEven'><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td CLASS='dataEven'></td></tr>\n",_("Scheduled"),time_indeterminate_scheduled_string,percent_time_indeterminate_scheduled);
		*/
		printf("<td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td CLASS='dataEven'></td></tr>\n", _("Nagios Not Running"),time_indeterminate_notrunning_string, percent_time_indeterminate_notrunning);
		printf("<tr CLASS='dataEven'><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td CLASS='dataEven'></td></tr>\n", _("Insufficient Data"),time_indeterminate_nodata_string, percent_time_indeterminate_nodata);
		printf("<tr CLASS='dataEven'><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%s</td><td CLASS='dataEven'>%2.3f%%</td><td CLASS='dataEven'></td></tr>\n", _("Total"),time_indeterminate_string, percent_time_indeterminate);
		...
		printf("<tr><td colspan=3></td></tr>\n");
		printf("<tr CLASS='dataOdd'><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>%s</td><td CLASS='dataOdd'>100.000%%</td><td CLASS='dataOdd'>100.000%%</td></tr>\n", _("All"), _("Total"),total_time_string);
		printf("</table>\n");
		printf("</DIV>\n");
...
#ifdef DEBUG
				printf("<li>%s[%d]: %d -> %d (%ld -> %ld) %d -> %d = %ld<br>\n",_("Matching in timerange"), weekday, temp_timerange->range_start, temp_timerange->range_end, temp_start, temp_end, start, end, end - start);
#endif
				if(end > start)
					temp_duration += end - start;
				}
