...
void display_info_table(char *title, int refresh, authdata *current_authdata) {
	...
	printf("%s: %s<BR>\n", _("Last Updated"),date_time);
	if(refresh == TRUE)
		printf("%s %d %s<br>\n", _("Updated every"),refresh_rate,_("seconds"));
	...
	if(current_authdata != NULL)
		printf("%s <i>%s</i><BR>\n", _("Logged in as"),(!strcmp(current_authdata->username, "")) ? "?" : current_authdata->username);

	if(nagios_process_state != STATE_OK)
		printf("<DIV CLASS='infoBoxBadProcStatus'>%s<BR> <A HREF='%s?type=%d'>%s</A> %s</DIV>", _("Warning: Monitoring process may not be running!"), EXTINFO_CGI, DISPLAY_PROCESS_INFO,_("Click"),_("more info"));

	if(result == ERROR)
		printf("<DIV CLASS='infoBoxBadProcStatus'>%s</DIV>",_("Warning: Could not read program status information!"));

	else {
		if(enable_notifications == FALSE)
			printf("<DIV CLASS='infoBoxBadProcStatus'>- %s</DIV>",_("Notifications are disabled"));
...
void display_nav_table(char *url, int archive) {
	...
	if(log_rotation_method != LOG_ROTATION_NONE) {
		...
		printf("<DIV CLASS='navBoxTitle'>%s</DIV>\n",_("Log File Navigation"));
	...
	printf("<BR><DIV CLASS='navBoxFile'>%s: %s</DIV>\n", archive_basename,_("File"));
	...
	/* an error occurred trying to stat() the file */
		...
		switch(errno) {
			...
			case ENAMETOOLONG: /* File name too long. */
				printf("<br /> %s %d %s %s.<br />", _("A stat call returned"), errno, _("while looking for the file"),filename);
				return;
...
void cgi_config_file_error(char *config_file) {
	...
	printf("<P><STRONG><FONT COLOR='RED'>'%s' </FONT></STRONG></P>\n", config_file,_("Error: Could not open CGI config file "));

	printf("<P>\n");
	printf("%s:\n",_("Here are some things you should check in order to resolve this error"));
	printf("</P>\n");
	...
	printf("<LI>%s.\n",_("Make sure the user your web server is running as has permission to read the CGI config file"));
...
void main_config_file_error(char *config_file) {
	...
	printf("<P><STRONG><FONT COLOR='RED'>%s '%s' for reading!</FONT></STRONG></P>\n", _("Error: Could not open main config file"),config_file);
	...
	printf("<LI>%s\n",_("Make sure you've installed a main config file in its proper location.  See the error message about for details on where the CGI is expecting to find the configuration file.  A sample main configuration file (named <b>nagios.cfg</b>) can be found in the <b>sample-config/</b> subdirectory of the Nagios source code distribution."));
	printf("<LI>%s.\n",_("Make sure the user your web server is running as has permission to read the main config file"));

	printf("</OL>\n");
	printf("</P>\n");

	printf("<P>\n");
	printf("%s.\n",_("Make sure you read the documentation on installing and configuring Nagios thoroughly before continuing.  If all else fails, try sending a message to one of the mailing lists.  More information can be found at <a href='http://www.nagios.org'>http://www.nagios.org</a>"));
	printf("</P>\n");
...
void object_data_error(void) {
	...
	printf("<P><STRONG><FONT COLOR='RED'>%s</FONT></STRONG></P>\n",_("Error: Could not read object configuration data!"));
	...
	printf("<LI>%s.\n",_("Verify configuration options using the <b>-v</b> command-line option to check for errors"));
	printf("<LI>%s.\n",_("Check the Nagios log file for messages relating to startup or status data errors"));

	printf("</OL>\n");
	printf("</P>\n");

	printf("<P>\n");
	printf("%s.\n",_("Make sure you read the documentation on installing, configuring and running Nagios thoroughly before continuing.  If all else fails, try sending a message to one of the mailing lists.  More information can be found at <a href='http://www.nagios.org'>http://www.nagios.org</a>"));
	printf("</P>\n");
...
void status_data_error(void) {
	...
	printf("<P>\n");
	printf("%s.\n",_("The most common cause of this error message (especially for new users), is the fact that Nagios is not actually running.  If Nagios is indeed not running, this is a normal error message.  It simply indicates that the CGIs could not obtain the current status of hosts and services that are being monitored.  If you've just installed things, make sure you read the documentation on starting Nagios"));
	printf("</P>\n");

	printf("<P>\n");
	printf("%s:\n",_("Some other things you should check in order to resolve this error include"));
	printf("</P>\n");
	...
	printf("<LI>%s.\n",_("Check the Nagios log file for messages relating to startup or status data errors"));
	printf("<LI>%s\n",_("Always verify configuration options using the <b>-v</b> command-line option before starting or restarting Nagios!"));
