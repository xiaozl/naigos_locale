...
void display_page_header(void) {
	...
	if(display_header == TRUE) {
		...
		if(show_all_hosts == FALSE) {
			printf("<a href='%s?host=all&max_width=%d&max_height=%d'>%s</a><BR>", STATUSMAP_CGI, max_image_width, max_image_height,_("View Status Map For All Hosts"));
			printf("<a href='%s?host=%s'>%s</a><BR>\n", STATUS_CGI, url_encode(host_name),_("View Status Detail For This Host"));
			}
		printf("<a href='%s?host=all'>",STATUS_CGI);
		printf(_("View Status Detail For All Hosts"));
		printf("</a><BR>\n");
		
		
		printf("<a href='%s?hostgroup=all'>%s</a><BR>\n", STATUS_CGI,_("View Status Overview For All Hosts"));
		...
		printf("<option value=%d %s>%s\n", LAYOUT_USER_SUPPLIED, (layout_method == LAYOUT_USER_SUPPLIED) ? "selected" : "",_("User-supplied coords"));
#endif
		printf("<option value=%d %s>%s\n", LAYOUT_SUBLAYERS, (layout_method == LAYOUT_SUBLAYERS) ? "selected" : "",_("Depth layers"));
		printf("<option value=%d %s>%s\n", LAYOUT_COLLAPSED_TREE, (layout_method == LAYOUT_COLLAPSED_TREE) ? "selected" : "",_("Collapsed tree"));
		printf("<option value=%d %s>%s\n", LAYOUT_BALANCED_TREE, (layout_method == LAYOUT_BALANCED_TREE) ? "selected" : "",_("Balanced tree"));
		printf("<option value=%d %s>%s\n", LAYOUT_CIRCULAR, (layout_method == LAYOUT_CIRCULAR) ? "selected" : "",_("Circular"));
		printf("<option value=%d %s>%s\n", LAYOUT_CIRCULAR_MARKUP, (layout_method == LAYOUT_CIRCULAR_MARKUP) ? "selected" : "",_("Circular (Marked Up)"));
		printf("<option value=%d %s>%s\n", LAYOUT_CIRCULAR_BALLOON, (layout_method == LAYOUT_CIRCULAR_BALLOON) ? "selected" : "",_("Circular (Balloon)"));
		printf("</select>\n");
		printf("</td>\n");
		...
		*/
		...
		printf("<tr><td CLASS='optBoxItem'>%s:<br>\n",_("Scaling factor"));
		...
		printf("</select>\n");
		printf("</td><td CLASS='optBoxItem' valign=top>%s:<br>",_("Scaling factor"));
		printf("<input type='radio' name='layermode' value='include' %s>%s<br>\n", (exclude_layers == FALSE) ? "CHECKED" : "",_("Include"));
		printf("<input type='radio' name='layermode' value='exclude' %s>%s\n", (exclude_layers == TRUE) ? "CHECKED" : "",_("Exclude"));
		printf("</td></tr>\n");
...
void draw_hosts(void) {
	...
	if(layout_method != LAYOUT_USER_SUPPLIED && draw_nagios_icon == TRUE) {
		...
		if(create_type == CREATE_IMAGE)
			draw_text(_("Nagios Process"), x1 + (DEFAULT_NODE_WIDTH / 2), y1 + DEFAULT_NODE_HEIGHT, color_black);
		}
...
void write_host_popup_text(host *hst) {
	...
	if(temp_status == NULL) {
		printf(_("Host status information not found"));
		return;
		}
	...
	printf("<tr><td class=\\\"popupText\\\">%s:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>", _("Name"),escape_string(hst->name));
	printf("<tr><td class=\\\"popupText\\\">%s:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>",_("Alias"), escape_string(hst->alias));
	printf("<tr><td class=\\\"popupText\\\">%s:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>", _("Address"),html_encode(hst->address, TRUE));
	printf("<tr><td class=\\\"popupText\\\">%s:</td><td class=\\\"popupText\\\"><b>",_("State"));

	/* get the status of the host (pending, up, down, or unreachable) */
	if(temp_status->status == HOST_DOWN) {
		printf("<font color=red>%s",_("Down"));
		if(temp_status->problem_has_been_acknowledged == TRUE)
			printf(_("(Acknowledged)"));
		printf("</font>");
		}

	else if(temp_status->status == HOST_UNREACHABLE) {
		printf("<font color=red>%s",_("Unreachable"));
		if(temp_status->problem_has_been_acknowledged == TRUE)
			printf(_("(Acknowledged)"));
		printf("</font>");
		}
		...
		printf("<font color=green>%s</font>",_("Up"));

	else if(temp_status->status == HOST_PENDING)
		printf("%s",_("Pending"));

	printf("</b></td></tr>");
	printf("<tr><td class=\\\"popupText\\\">%s:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>",_("Status Information"), (temp_status->plugin_output == NULL) ? "" : temp_status->plugin_output);
	...
	printf("<tr><td class=\\\"popupText\\\">%s:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>", _("State Duration"),state_duration);

	get_time_string(&temp_status->last_check, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
	printf("<tr><td class=\\\"popupText\\\">%s:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>", _("Last Status Check"),(temp_status->last_check == (time_t)0) ? "N/A" : date_time);
	get_time_string(&temp_status->last_state_change, date_time, (int)sizeof(date_time), SHORT_DATE_TIME);
	printf("<tr><td class=\\\"popupText\\\"%s:</td><td class=\\\"popupText\\\"><b>%s</b></td></tr>", _("Last State Change"),(temp_status->last_state_change == (time_t)0) ? "N/A" : date_time);

	printf("<tr><td class=\\\"popupText\\\">%s:</td><td class=\\\"popupText\\\"><b>",_("Parent Host(s)"));
	if(hst->parent_hosts == NULL)
		printf(_("None (This is a root host)"));
	else {
	...
	printf("<tr><td class=\\\"popupText\\\">%s:</td><td class=\\\"popupText\\\"><b>",_("Immediate Child Hosts"));
	printf("%d", number_of_immediate_child_hosts(hst));
	printf("</b></td></tr>");

	printf("</table>");

	printf("<br><b><u>%s:</u></b><br>",_("Services"));

	service_totals = get_servicestatus_count(hst->name, SERVICE_OK);
	if(service_totals > 0)
		printf("- <font color=green>%d %s</font><br>", service_totals,_("ok"));
	service_totals = get_servicestatus_count(hst->name, SERVICE_CRITICAL);
	if(service_totals > 0)
		printf("- <font color=red>%d %s</font><br>", service_totals,_("critical"));
	service_totals = get_servicestatus_count(hst->name, SERVICE_WARNING);
	if(service_totals > 0)
		printf("- <font color=orange>%d %s</font><br>", service_totals,_("warning"));
	service_totals = get_servicestatus_count(hst->name, SERVICE_UNKNOWN);
	if(service_totals > 0)
		printf("- <font color=orange>%d %s</font><br>", service_totals,_("unknown"));
	service_totals = get_servicestatus_count(hst->name, SERVICE_PENDING);
	if(service_totals > 0)
		printf("- %d %s<br>", service_totals,_("pending"));
